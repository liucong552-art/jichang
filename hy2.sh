...
need_basic_tools() {
  ...
  apt-get install -y --no-install-recommends \
    ca-certificates curl wget openssl python3 snapd nftables iproute2 util-linux coreutils grep sed gawk logrotate systemd
}
...
# 4) 使用 certbot standalone 申请证书，需要 TCP/80 可达。
#
# 客户端连接域名
HY_DOMAIN=
...
HY2_RENEW_HOOK="/etc/letsencrypt/renewal-hooks/deploy/zz-hy2-managed-reload.sh"
...
hy2_parse_port_from_cfg() {
  ...
  awk '
    /^[[:space:]]*listen:/ {
      gsub(/"/,"",$2)
      if (match($0, /([0-9]+)[[:space:]]*$/, m)) { print m[1]; exit }
    }
  ' "$cfg"
}
...
ensure_modern_certbot() {
  ...
  if command -v certbot >/dev/null 2>&1; then
    current_ver="$(certbot --version 2>/dev/null | awk '{print $2}' | tr -d ',' || true)"
  fi
  if [[ -n "$current_ver" && "$current_ver" != "2.1.0-4" ]]; then
    return 0
  fi

  export DEBIAN_FRONTEND=noninteractive
  apt-get update -o Acquire::Retries=3
  apt-get remove -y certbot python3-certbot python3-acme python3-josepy python3-openssl python3-cryptography python3-configargparse python3-configobj python3-distro python3-parsedatetime python3-rfc3339 python3-tz python3-cffi-backend >/dev/null 2>&1 || true
  apt-get install -y --no-install-recommends snapd >/dev/null 2>&1
  systemctl enable --now snapd.socket >/dev/null 2>&1 || true
  systemctl restart snapd.socket >/dev/null 2>&1 || true
  command -v snap >/dev/null 2>&1 || hy2_die "snapd 安装失败，无法升级 certbot"
  snap install core >/dev/null 2>&1 || true
  snap refresh core >/dev/null 2>&1 || true
  snap install --classic certbot >/dev/null 2>&1 || snap refresh certbot >/dev/null 2>&1 || true
  ln -sf /snap/bin/certbot /usr/bin/certbot
  certbot --version >/dev/null 2>&1 || hy2_die "certbot 不可用"
}
