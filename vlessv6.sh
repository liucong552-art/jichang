#!/usr/bin/env bash
# Modified from the user-provided vless (1).sh.
# Adds IPv6 temporary-node support while keeping the original IPv4 path as default.
set -Eeuo pipefail
...
# The domain clients use to connect to this VPS over IPv4.
PUBLIC_DOMAIN=

# Optional: domain clients use to connect to temporary nodes over IPv6.
# Set this to a domain with AAAA record pointing to this VPS IPv6.
PUBLIC_IPV6_DOMAIN=
...
force_xray_run_as_root() {
  ...
  local dropin_file="${dropin_dir}/99-run-as-root.conf"

  mkdir -p "$dropin_dir"

  if [[ -f "$unit" ]]; then
    cp -a "$unit" "${unit}.bak.$(date +%F-%H%M%S)"

    if grep -q '^User=' "$unit"; then
      sed -i 's/^User=.*/User=root/' "$unit"
    else
      sed -i '/^\[Service\]/a User=root' "$unit"
    fi

    if grep -q '^Group=' "$unit"; then
      sed -i 's/^Group=.*/Group=root/' "$unit"
    else
      sed -i '/^\[Service\]/a Group=root' "$unit"
    fi
...
vr_get_public_ipv6() {
  ...
  ip="$(ip -6 addr show scope global 2>/dev/null | awk '/inet6/ {print $2}' | cut -d/ -f1 | head -n1 | tr -d ' \n\r' || true)"
  if [[ -n "$ip" ]] && vr_is_public_ipv6 "$ip"; then
    printf '%s\n' "$ip"
    return 0
  fi
...
vr_require_domain_aaaa_points_here() {
  local domain="$1" current_ip="$2" ok=1 ip
  mapfile -t resolved < <(vr_resolve_domain_ipv6s "$domain")
  (( ${#resolved[@]} > 0 )) || vr_die "无法解析 PUBLIC_IPV6_DOMAIN=${domain} 的 IPv6 AAAA 记录"
  for ip in "${resolved[@]}"; do
    if vr_ipv6_equal "$ip" "$current_ip"; then
      ok=0
      break
    fi
  done
  (( ok == 0 )) || vr_die "PUBLIC_IPV6_DOMAIN=${domain} 的 AAAA 记录未指向当前 VPS IPv6=${current_ip}；当前解析: ${resolved[*]}"
}
...
vr_collect_orphan_temp_tags_from_aux() {
  ...
  for meta in "$VR_QUOTA_STATE_DIR"/*.env "$VR_IPLIMIT_STATE_DIR"/*.env; do
    ...
    [[ -n "$owner_tag" ]] || continue
    [[ "$owner_tag" =~ ^[A-Za-z0-9._-]+$ ]] || continue
...
vr_il_rebuild_port() {
  local port="$1" ip_limit="$2" sticky_seconds="$3" ip_version="${4:-4}"
...
add rule inet ${VR_IL_TABLE} ${VR_IL_INPUT_CHAIN} tcp dport ${port} ip6 saddr @$(vr_il_set_name "$port" 6) update @$(vr_il_set_name "$port" 6) { ip6 saddr timeout ${sticky_seconds}s } accept comment "$(vr_il_comment_refresh "$port")"
add rule inet ${VR_IL_TABLE} ${VR_IL_INPUT_CHAIN} tcp dport ${port} add @$(vr_il_set_name "$port" 6) { ip6 saddr timeout ${sticky_seconds}s } accept comment "$(vr_il_comment_claim "$port")"
...
add rule inet ${VR_IL_TABLE} ${VR_IL_INPUT_CHAIN} tcp dport ${port} ip saddr @$(vr_il_set_name "$port" 4) update @$(vr_il_set_name "$port" 4) { ip saddr timeout ${sticky_seconds}s } accept comment "$(vr_il_comment_refresh "$port")"
add rule inet ${VR_IL_TABLE} ${VR_IL_INPUT_CHAIN} tcp dport ${port} add @$(vr_il_set_name "$port" 4) { ip saddr timeout ${sticky_seconds}s } accept comment "$(vr_il_comment_claim "$port")"
add rule inet ${VR_IL_TABLE} ${VR_IL_INPUT_CHAIN} tcp dport ${port} drop comment "$(vr_il_comment_drop "$port")"
EOF_RULES
    then
      vr_il_failsafe_block_port "$port"
...
vr_il_state() {
  ...
  ip_version="$(vr_meta_get "$meta" IP_VERSION 2>/dev/null || true)"
  ip_version="${ip_version:-4}"
  if nft list set inet "$VR_IL_TABLE" "$(vr_il_set_name "$port" "$ip_version")" >/dev/null 2>&1; then
    printf 'active\n'
  else
    printf 'stale\n'
  fi
...
/usr/local/sbin/vless_temp_sub.sh >/dev/null 2>&1 || true
systemctl daemon-reload >/dev/null 2>&1 || true
__VR_FILE_22__
chmod 755 '/usr/local/sbin/vless_cleanup_one.sh'

cat >'/usr/local/sbin/vless_temp_sub.sh' <<'__VR_FILE_22B__'
#!/usr/bin/env bash
set -Eeuo pipefail
trap 'echo "❌ ${BASH_SOURCE[0]}:${LINENO}: ${BASH_COMMAND}" >&2' ERR
...
sort -u "$TMP" >"$OUT_RAW"
vr_base64_one_line <"$OUT_RAW" >"$OUT_B64"
chmod 600 "$OUT_RAW" "$OUT_B64" 2>/dev/null || true
...
if [[ "$IP_VERSION" == "6" ]]; then
  ...
  SERVER_IPV6="$(vr_get_public_ipv6 || true)"
  [[ -n "$SERVER_IPV6" ]] || vr_die "无法检测到可用的公网 IPv6"
  vr_require_domain_aaaa_points_here "$PUBLIC_IPV6_DOMAIN" "$SERVER_IPV6"
...
UNIT
  ...
  VLESS_URL="vless://${UUID}@${URL_HOST}:${PORT}?type=tcp&security=reality&encryption=none&flow=xtls-rprx-vision&sni=${REALITY_SNI}&fp=chrome&pbk=${PBK_Q}&sid=${SHORT_ID}#${TAG}"
  printf '%s\n' "$VLESS_URL" >"$URL_FILE"
