...
fmt_expire_cn() { local exp="${1:-}"; if [[ "$exp" =~ ^[0-9]+$ ]] && (( exp > 0 )); then TZ='Asia/Shanghai' date -d "@$exp" '+%m-%d %H:%M' 2>/dev/null || echo -; else echo -; fi; }
quota_cells() {
  ...
  if [[ -f "$meta" ]]; then
    ...
    limit_bytes="$(meta_get "$meta" LIMIT_BYTES || true)"; saved_used="$(meta_get "$meta" SAVED_USED_BYTES || true)"; remain_bytes="$(meta_get "$meta" REMAIN_BYTES || true)"; mode="$(meta_get "$meta" MODE || true)"
    [[ "$limit_bytes" =~ ^[0-9]+$ ]] || limit_bytes=0; [[ "$saved_used" =~ ^[0-9]+$ ]] || saved_used=0; [[ "$remain_bytes" =~ ^[0-9]+$ ]] || remain_bytes="$limit_bytes"; mode="${mode:-quota}"
    ...
    if [[ "$mode" != "quota" || "$limit_bytes" == "0" ]]; then
      ...
      if (( remain_bytes == 0 )); then total_used="$limit_bytes"; left_b=0; fi
...
echo; printf '%s\n' '=== VLESS AUDIT ==='; print_sep; render_row "NAME" "STATE" "PORT" "RDY" "Q" "LIMIT" "USED" "LEFT" "USE%" "TTL" "EXPIRE"; print_sep
MAIN_PORT="$(get_main_port)"; MAIN_STATE="$(systemctl is-active xray.service 2>/dev/null || echo unknown)"; MAIN_READY=no; [[ "$MAIN_STATE" == active ]] && tcp_port_listening "$MAIN_PORT" && MAIN_READY=yes
...
get_counter_bytes() { local obj="${1:-}"; nft list counter inet portquota "$obj" 2>/dev/null | awk '/bytes/{for(i=1;i<=NF;i++) if($i=="bytes"){print $(i+1);exit}}'; }
get_quota_used_bytes() { local obj="${1:-}"; nft list quota inet portquota "$obj" 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="used"){print $(i+1); exit}}'; }
PORT="${1:-}"; BYTES_ARG="${2:-0}"; META="/etc/portquota/pq-${PORT}.meta"; MAX_IPS="${3:-${MAX_IPS:-}}"; IP_TIMEOUT_SECONDS="${4:-${IP_TIMEOUT_SECONDS:-}}"
[[ "$PORT" =~ ^[0-9]+$ ]] || { echo "❌ 端口非法" >&2; exit 1; }
...
if (( LIMIT_BYTES > 0 && TOTAL_USED >= LIMIT_BYTES )); then [[ -f "$META" ]] && { meta_set "$META" SAVED_USED_BYTES "$LIMIT_BYTES"; meta_set "$META" REMAIN_BYTES 0; }; REMAIN_BYTES=0; fi
LOCK="/run/portquota.lock"; if [[ "${PQ_LOCK_HELD:-0}" != 1 ]]; then exec 9>"$LOCK"; flock -w 10 9; fi
NEED=0
...
if (( NEED == 1 )); then
  if (( LIVE_USED > 0 && LIMIT_BYTES > 0 )); then SAVED_NEW=$((SAVED_USED_BYTES + LIVE_USED)); (( SAVED_NEW > LIMIT_BYTES )) && SAVED_NEW="$LIMIT_BYTES"; REMAIN_BYTES=$((LIMIT_BYTES - SAVED_NEW)); (( REMAIN_BYTES < 0 )) && REMAIN_BYTES=0; [[ -f "$META" ]] && { meta_set "$META" SAVED_USED_BYTES "$SAVED_NEW"; meta_set "$META" REMAIN_BYTES "$REMAIN_BYTES"; }; fi
...
fmt_epoch() { local ts="${1:-}"; if [[ "$ts" =~ ^[0-9]+$ ]] && (( ts > 0 )); then date -d "@$ts" '+%m-%d %H:%M' 2>/dev/null || echo "$ts"; else echo -; fi; }
...
for META in /etc/portquota/pq-*.meta; do
  PORT="$(meta_get "$META" PORT || true)"; LIMIT_BYTES="$(meta_get "$META" LIMIT_BYTES || true)"; SAVED_USED="$(meta_get "$META" SAVED_USED_BYTES || true)"; REMAIN_BYTES="$(meta_get "$META" REMAIN_BYTES || true)"; MODE="$(meta_get "$META" MODE || true)"; AUTO_RESET="$(meta_get "$META" AUTO_RESET || true)"; NEXT_RESET_EPOCH="$(meta_get "$META" NEXT_RESET_EPOCH || true)"; RESET_COUNT="$(meta_get "$META" RESET_COUNT || true)"; EXPIRE_EPOCH="$(meta_get "$META" EXPIRE_EPOCH || true)"; MAX_IPS="$(meta_get "$META" MAX_IPS || true)"; IP_TIMEOUT_SECONDS="$(meta_get "$META" IP_TIMEOUT_SECONDS || true)"
  [[ -n "$PORT" ]] || continue
  [[ "$LIMIT_BYTES" =~ ^[0-9]+$ ]] || LIMIT_BYTES=0; [[ "$SAVED_USED" =~ ^[0-9]+$ ]] || SAVED_USED=0; [[ "$REMAIN_BYTES" =~ ^[0-9]+$ ]] || REMAIN_BYTES="$LIMIT_BYTES"
  OUT_B="$(get_counter_bytes "pq_out_${PORT}" || true)"; [[ "$OUT_B" =~ ^[0-9]+$ ]] || OUT_B=0
  IN_B="$(get_counter_bytes "pq_in_${PORT}" || true)"; [[ "$IN_B" =~ ^[0-9]+$ ]] || IN_B=0
  TOTAL_B=$((OUT_B + IN_B))
...
for META in /etc/portquota/pq-*.meta; do
  ...
  (( NEW_SAVED > LIMIT_BYTES )) && NEW_SAVED="$LIMIT_BYTES"
  REMAIN=$((LIMIT_BYTES - NEW_SAVED))
  (( REMAIN < 0 )) && REMAIN=0
