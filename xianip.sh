...
NEW_CFG="$CONFIG_DIR/config.json.new.$$"
cat >"$NEW_CFG" <<CONF
{
...
if ! xray_check_config "$NEW_CFG"; then
  echo "❌ 新生成的 xray 配置未通过校验，已停止覆盖旧配置" >&2
  rm -f "$NEW_CFG"
  exit 1
fi
...
meta_set() { local file="$1" key="$2" value="$3" tmp="${file}.tmp.$$"; awk -F= -v k="$key" -v v="$value" 'BEGIN{d=0}$1==k{print k"="v;d=1;next}{print}END{if(!d)print k"="v}' "$file" > "$tmp" && mv "$tmp" "$file"; }
port_is_listening() { local port="$1"; ss -ltnH 2>/dev/null | awk -v p="$port" '{ n=split($4,a,":"); if (a[n] == p) {found=1; exit} } END { exit(found ? 0 : 1) }'; }
xray_check_config() { /usr/local/bin/xray run -test -config "$1" >/dev/null 2>&1; }
read_port_from_cfg() {
  local cfg="$1"
  python3 - "$cfg" <<'PY' 2>/dev/null || true
import json,sys
...
if [[ "$POLICY" == "1" && "$PORT" =~ ^[0-9]+$ ]]; then
  ...
  if [[ ! -f "/etc/portquota/pq-${PORT}.meta" ]] && [[ -x /usr/local/sbin/pq_add.sh ]]; then
    if ! CREATE_EPOCH="$CREATE_EPOCH" MAX_IPS="${MAX_IPS:-0}" IP_TIMEOUT_SECONDS="${IP_TIMEOUT_SECONDS:-90}" /usr/local/sbin/pq_add.sh "$PORT" "${PQ_GIB:-0}" "${DURATION_SECONDS:-0}" "$EXP" >/dev/null 2>&1; then
      echo "❌ 恢复失败：端口策略重建失败" >&2
      exit 1
    fi
  elif [[ -x /usr/local/sbin/pq_ensure_port.sh ]]; then
    if ! PQ_LOCK_HELD=0 MAX_IPS="${MAX_IPS:-0}" IP_TIMEOUT_SECONDS="${IP_TIMEOUT_SECONDS:-90}" /usr/local/sbin/pq_ensure_port.sh "$PORT" "$LIMIT_BYTES" >/dev/null 2>&1; then
...
trap cleanup EXIT
{
  ...
  if (( MAX_IPS > 0 )); then
    ...
    echo "add rule inet portquota down_out ${PROTO} sport $PORT update @pq_ip_$PORT { ip daddr timeout ${IP_TIMEOUT_SECONDS}s } comment \"pq-ip-refresh-out-$PORT\""
    echo "add rule inet portquota down_out ${PROTO} sport $PORT ip daddr != @pq_ip_$PORT drop comment \"pq-ip-enforce-out-$PORT\""
  fi
  if (( BYTES > 0 )); then
...
obj_exists() { local kind="$1" name="$2"; nft list "$kind" inet portquota "$name" >/dev/null 2>&1; }
rule_exists() {
  local chain="$1" comment="$2" expect1="${3:-}" expect2="${4:-}" line
  line="$(nft -a list chain inet portquota "$chain" 2>/dev/null | grep -F "comment \"$comment\"" | head -n1 || true)"
  [[ -n "$line" ]] || return 1
  [[ -z "$expect1" || "$line" == *"$expect1"* ]] || return 1
  [[ -z "$expect2" || "$line" == *"$expect2"* ]]
}
quota_matches() {
  local name="$1" bytes="$2" line
  line="$(nft list quota inet portquota "$name" 2>/dev/null | tr '\n' ' ' || true)"
  [[ -n "$line" ]] && [[ "$line" == *"over ${bytes} bytes"* || "$line" == *"over \"${bytes}\" bytes"* ]]
}
set_matches() {
  local name="$1" size="$2" timeout="$3" line
  line="$(nft list set inet portquota "$name" 2>/dev/null | tr '\n' ' ' || true)"
  [[ -n "$line" ]] && [[ "$line" == *"size ${size}"* ]] && [[ "$line" == *"timeout ${timeout}s"* ]]
}
delete_rules_by_comment() {
  ...
  while read -r h; do
    [[ "$h" =~ ^[0-9]+$ ]] || continue
    nft delete rule inet portquota "$chain" handle "$h" >/dev/null 2>&1 || true
  done <<< "$handles"
}
ensure_rule() {
  local chain="$1" comment="$2"
  shift 2
  if ! rule_exists "$chain" "$comment" "$@"; then
    nft add rule inet portquota "$chain" "$@" comment "$comment" >/dev/null 2>&1
    CHANGED=1
  fi
}
PORT="${1:-}"
...
if [[ "${PQ_LOCK_HELD:-0}" != "1" ]]; then exec 9>"$LOCK"; flock -w 10 9; fi
CHANGED=0
if ! nft list table inet portquota >/dev/null 2>&1; then nft add table inet portquota; CHANGED=1; fi
if ! nft list chain inet portquota down_out >/dev/null 2>&1; then nft add chain inet portquota down_out '{ type filter hook output priority filter; policy accept; }'; CHANGED=1; fi
if ! nft list chain inet portquota up_in >/dev/null 2>&1; then nft add chain inet portquota up_in '{ type filter hook input priority filter; policy accept; }'; CHANGED=1; fi
obj_exists counter "pq_out_$PORT" || { nft add counter inet portquota "pq_out_$PORT" >/dev/null 2>&1; CHANGED=1; }
obj_exists counter "pq_in_$PORT" || { nft add counter inet portquota "pq_in_$PORT" >/dev/null 2>&1; CHANGED=1; }
ensure_rule down_out "pq-count-out-$PORT" ${PROTO} sport "$PORT" counter name "pq_out_$PORT"
ensure_rule up_in "pq-count-in-$PORT" ${PROTO} dport "$PORT" counter name "pq_in_$PORT"
if (( BYTES > 0 )); then
  if ! obj_exists quota "pq_quota_$PORT"; then
    nft add quota inet portquota "pq_quota_$PORT" "{ over $BYTES bytes }" >/dev/null 2>&1
    CHANGED=1
  elif ! quota_matches "pq_quota_$PORT" "$BYTES"; then
    echo "[pq_ensure_port] quota bytes drift for ${PORT}; refuse destructive reset" >&2
    exit 2
  fi
  ensure_rule down_out "pq-drop-out-$PORT" ${PROTO} sport "$PORT" quota name "pq_quota_$PORT" drop
  ensure_rule up_in "pq-drop-in-$PORT" ${PROTO} dport "$PORT" quota name "pq_quota_$PORT" drop
else
  delete_rules_by_comment down_out "pq-drop-out-$PORT"
  delete_rules_by_comment up_in "pq-drop-in-$PORT"
  if obj_exists quota "pq_quota_$PORT"; then
    nft delete quota inet portquota "pq_quota_$PORT" >/dev/null 2>&1 || true
    CHANGED=1
  fi
fi
if (( MAX_IPS > 0 )); then
  if ! obj_exists set "pq_ip_$PORT"; then
    nft add set inet portquota "pq_ip_$PORT" "{ type ipv4_addr; flags timeout,dynamic; timeout ${IP_TIMEOUT_SECONDS}s; size ${MAX_IPS}; }" >/dev/null 2>&1
    CHANGED=1
  elif ! set_matches "pq_ip_$PORT" "$MAX_IPS" "$IP_TIMEOUT_SECONDS"; then
    echo "[pq_ensure_port] ip set drift for ${PORT}; refuse destructive reset" >&2
    exit 3
  fi
  ensure_rule up_in "pq-ip-refresh-in-$PORT" ${PROTO} dport "$PORT" update @"pq_ip_$PORT" '{' ip saddr timeout "${IP_TIMEOUT_SECONDS}"s '}'
  ensure_rule up_in "pq-ip-enforce-in-$PORT" ${PROTO} dport "$PORT" ip saddr '!=' @"pq_ip_$PORT" drop
  ensure_rule down_out "pq-ip-refresh-out-$PORT" ${PROTO} sport "$PORT" update @"pq_ip_$PORT" '{' ip daddr timeout "${IP_TIMEOUT_SECONDS}"s '}'
  ensure_rule down_out "pq-ip-enforce-out-$PORT" ${PROTO} sport "$PORT" ip daddr '!=' @"pq_ip_$PORT" drop
else
  delete_rules_by_comment up_in "pq-ip-refresh-in-$PORT"
  delete_rules_by_comment up_in "pq-ip-enforce-in-$PORT"
  delete_rules_by_comment down_out "pq-ip-refresh-out-$PORT"
  delete_rules_by_comment down_out "pq-ip-enforce-out-$PORT"
  if obj_exists set "pq_ip_$PORT"; then
    nft delete set inet portquota "pq_ip_$PORT" >/dev/null 2>&1 || true
    CHANGED=1
  fi
fi
if (( CHANGED == 1 )); then
...
if [[ "$MODE" == "quota" ]] && (( LIMIT_BYTES > 0 )) && (( SERVICE_SECONDS > RESET_INTERVAL_SECONDS )) && (( EXPIRE_EPOCH > CREATE_EPOCH )); then
  ...
  if (( NEXT_RESET_EPOCH <= 0 )); then
    if (( LAST_RESET_EPOCH > 0 )); then
      NEXT_RESET_EPOCH=$((LAST_RESET_EPOCH + RESET_INTERVAL_SECONDS))
    else
...
for META in /etc/portquota/pq-*.meta; do
  ...
  [[ "$MAX_IPS" =~ ^[0-9]+$ ]] || MAX_IPS=0
  [[ "$IP_TIMEOUT_SECONDS" =~ ^[0-9]+$ ]] || IP_TIMEOUT_SECONDS=90
  if ! PQ_LOCK_HELD=1 MAX_IPS="$MAX_IPS" IP_TIMEOUT_SECONDS="$IP_TIMEOUT_SECONDS" /usr/local/sbin/pq_ensure_port.sh "$PORT" "$LIMIT_BYTES" >/dev/null 2>&1; then
