#!/usr/bin/env bash
# Debian 12 一键部署脚本（重构版）
# 原始脚本: xianip.sh
# 重构改动:
#   - 提取共享库 /usr/local/lib/vless-common.sh，消除 ~500 行重复函数定义
#   - 修复 pq_reset.sh heredoc 截断 bug（原 2252-2265 行残留代码）
#   - 统一路径常量、错误处理、输入校验
#   - pq_save_state.sh: track 模式下不再用 LIMIT_BYTES 截断 NEW_SAVED
#   - 所有子脚本通过 source 共享库获取公共函数
...
check_debian12() {
  if [[ "$(id -u)" -ne 0 ]]; then
    echo "❌ 请以 root 运行本脚本"; exit 1
  fi
  local codename
  codename=$(grep -E "^VERSION_CODENAME=" /etc/os-release 2>/dev/null | cut -d= -f2 | tr -d '"')
  if [[ "$codename" != "bookworm" ]]; then
    echo "❌ 本脚本仅适用于 Debian 12 (bookworm)，当前: ${codename:-未知}"; exit 1
  fi
}
...
install_common_lib() {
  echo "🧩 写入共享库 /usr/local/lib/vless-common.sh ..."
  mkdir -p /usr/local/lib
  cat >/usr/local/lib/vless-common.sh <<'EOF_LIB'
#!/usr/bin/env bash
# vless-common.sh — 共享工具函数库
# 所有 VLESS / portquota 子脚本通过 source 本文件获取公共函数

# ---------- 路径常量 ----------
XRAY_DIR="${XRAY_DIR:-/usr/local/etc/xray}"
XRAY_BIN="${XRAY_BIN:-$(command -v xray 2>/dev/null || echo /usr/local/bin/xray)}"
PQ_DIR="${PQ_DIR:-/etc/portquota}"
PQ_LOCK="${PQ_LOCK:-/run/portquota.lock}"
VLESS_LOCK="${VLESS_LOCK:-/run/vless-temp.lock}"
MANAGED_DIR="${MANAGED_DIR:-/run/vless-managed-stop}"
PROTO="${PROTO:-tcp}"

# ---------- meta 文件读写 ----------
meta_get() {
  local file="$1" key="$2"
  awk -F= -v k="$key" '$1==k {sub($1"=","",$0); print; exit}' "$file" 2>/dev/null || true
}
meta_set() {
  local file="$1" key="$2" value="$3" tmp="${file}.tmp.$$"
  awk -F= -v k="$key" -v v="$value" \
    'BEGIN{d=0} $1==k{print k"="v;d=1;next} {print} END{if(!d)print k"="v}' \
    "$file" > "$tmp" && mv "$tmp" "$file"
}
write_meta() {
  local dst="$1" tmp="${1}.tmp.$$"
  cat >"$tmp"; chmod 600 "$tmp" 2>/dev/null || true; mv -f "$tmp" "$dst"
}
...
cfg_get() {
  local file="$1" key="$2"
  awk -F= -v k="$key" '
    $1==k { sub($1"=",""); gsub(/^[[:space:]]+|[[:space:]]+$/, "", $0); gsub(/^"|"$/, "", $0); print; exit }
  ' "$file"
}
...
tcp_port_listening() { port_is_listening "$@"; }

wait_unit_running() {
  local unit="$1" tries="${2:-20}" delay="${3:-0.25}" i state sub
  for ((i=1; i<=tries; i++)); do
    ...
    [[ "$state" == "active" && ( "$sub" == "running" || "$sub" == "listening" || -z "$sub" ) ]] && return 0
    [[ "$state" == "failed" || "$state" == "inactive" || "$state" == "deactivating" ]] && return 1
    sleep "$delay"
  done
...
wait_port_listening() {
  local port="$1" tries="${2:-20}" delay="${3:-0.25}" i
  for ((i=1; i<=tries; i++)); do
    port_is_listening "$port" && return 0; sleep "$delay"
  done
  return 1
}
...
# ---------- Xray 工具 ----------
xray_check_config() { "$XRAY_BIN" run -test -format=json -config "$1" >/dev/null 2>&1; }
...
managed_restart() {
  local unit="$1" flag="$2"
  : >"$flag"
  if ! systemctl restart "$unit" >/dev/null 2>&1; then rm -f "$flag"; return 1; fi
  rm -f "$flag"
}
managed_disable_now() {
  local unit="$1" flag="$2"
  : >"$flag"
  if ! systemctl disable --now "$unit" >/dev/null 2>&1; then rm -f "$flag"; return 1; fi
  rm -f "$flag"
}

# ---------- nftables 工具 ----------
obj_exists() { nft list "$1" inet portquota "$2" >/dev/null 2>&1; }
get_counter_bytes() { nft list counter inet portquota "$1" 2>/dev/null | awk '/bytes/{for(i=1;i<=NF;i++) if($i=="bytes"){print $(i+1);exit}}'; }
get_quota_used_bytes() { nft list quota inet portquota "$1" 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="used"){print $(i+1);exit}}'; }
...
cleanup_old_policy_port() {
  ...
  port_referenced_elsewhere "$old_port" "$self_meta" && return 0
  if [[ -x /usr/local/sbin/pq_del.sh ]] && { [[ -f "${PQ_DIR}/pq-${old_port}.meta" ]] || nft list table inet portquota >/dev/null 2>&1; }; then
    /usr/local/sbin/pq_del.sh "$old_port" >/dev/null 2>&1 || true
  fi
}
...
fmt_left() {
  local exp="${1:-}" now=$(date +%s)
  if [[ "$exp" =~ ^[0-9]+$ ]]; then
    local left=$((exp-now))
    if (( left <= 0 )); then echo expired
    else printf "%02dd%02dh%02dm" $((left/86400)) $(((left%86400)/3600)) $(((left%3600)/60)); fi
  else echo -; fi
}
fmt_expire_cn() {
  local exp="${1:-}"
  if [[ "$exp" =~ ^[0-9]+$ ]] && (( exp > 0 )); then
    TZ='Asia/Shanghai' date -d "@$exp" '+%m-%d %H:%M' 2>/dev/null || echo -
  else echo -; fi
}
fmt_epoch() {
  local ts="${1:-}"
  if [[ "$ts" =~ ^[0-9]+$ ]] && (( ts > 0 )); then
    date -d "@$ts" '+%m-%d %H:%M' 2>/dev/null || echo "$ts"
  else echo -; fi
}

# ---------- 输入校验 ----------
sanitize_one_line() { [[ "$1" != *$'\n'* && "$1" != *$'\r'* ]]; }
assert_positive_int() { [[ "${1:-}" =~ ^[0-9]+$ ]] && (( $1 > 0 )); }
assert_non_negative_int() { [[ "${1:-}" =~ ^[0-9]+$ ]]; }
EOF_LIB
  chmod 644 /usr/local/lib/vless-common.sh
}
...
source /usr/local/lib/vless-common.sh
check_debian12
...
[[ -f "$BACKPORTS_FILE" ]] && cp "$BACKPORTS_FILE" "${BACKPORTS_FILE}.bak.$(date +%F-%H%M%S)"
...
[[ -f "$CONF_FILE" ]] || { echo "❌ 缺少配置文件: $CONF_FILE"; exit 1; }
...
if [[ -z "$PUBLIC_DOMAIN" || "$PUBLIC_DOMAIN" == "your.domain.com" ]]; then
  echo "❌ 请先编辑 $CONF_FILE，至少把 PUBLIC_DOMAIN 改成你自己的域名"; exit 1
fi
if ! [[ "$PORT" =~ ^[0-9]+$ ]] || (( PORT < 1 || PORT > 65535 )); then
  echo "❌ PORT 无效：$PORT"; exit 1
fi

if ! getent ahostsv4 "$PUBLIC_DOMAIN" >/dev/null 2>&1; then
  echo "❌ 域名未解析到 IPv4: $PUBLIC_DOMAIN"; exit 1
fi
if ! domain_hits_local_ipv4 "$PUBLIC_DOMAIN"; then
  echo "⚠️  警告：PUBLIC_DOMAIN 已解析但未命中本机 global IPv4，客户端可能无法连接。"
fi
...
[[ -n "$PRIVATE_KEY" && -n "$PUBLIC_KEY" ]] || { echo "❌ 无法解析 Reality 密钥"; exit 1; }
...
cat >"$NEW_CFG" <<CONF
{
  ...
  "inbounds": [{
    ...
    "streamSettings": {
      ...
      "realitySettings": {
        "show": false, "target": "$REALITY_DEST", "xver": 0,
        "serverNames": ["$REALITY_SNI"], "privateKey": "$PRIVATE_KEY", "shortIds": ["$SHORT_ID"]
      }
    },
...
CONF
chmod 600 "$NEW_CFG"
xray_check_config "$NEW_CFG" || { echo "❌ 配置校验失败: $NEW_CFG" >&2; exit 1; }
...
NODE_NAME=$NODE_NAME
META
chmod 600 "$TMP_RUNTIME"; mv -f "$TMP_RUNTIME" "$RUNTIME_META"
...
[[ -f "$MANAGED_FLAG" ]] && exit 0

PORT=""
[[ -f "$META" ]] && PORT="$(meta_get "$META" PORT || true)"
[[ -z "${PORT:-}" && -f "$CFG" ]] && PORT="$(read_port_from_cfg "$CFG" || true)"
...
if [[ "$FORCE" != "1" && -f "$META" ]]; then
  ...
  if [[ -n "${EXPIRE_EPOCH:-}" && "$EXPIRE_EPOCH" =~ ^[0-9]+$ ]]; then
    NOW=$(date +%s); (( EXPIRE_EPOCH > NOW )) && exit 0
  fi
fi
...
if [[ "${VLESS_LOCK_HELD:-0}" != "1" ]]; then
  exec 9>"$VLESS_LOCK"; flock -w 10 9
fi
...
if [[ "$ACTIVE_STATE" == "active" || "$ACTIVE_STATE" == "activating" || "$SUB_STATE" == "running" || "$SUB_STATE" == "start" ]]; then
  : >"$MANAGED_FLAG"
  timeout 15 systemctl stop "$UNIT_NAME" >/dev/null 2>&1 || {
...
META="${XRAY_DIR}/${TAG}.meta"
[[ -x "$XRAY_BIN" ]] || { echo "[vless_run_temp] xray not found" >&2; exit 1; }
...
if (( REMAIN <= 0 )); then
  FORCE=1 /usr/local/sbin/vless_poststop_cleanup.sh "$TAG" 2>/dev/null || true; exit 0
fi

if [[ "${POLICY:-0}" == "1" ]]; then
  LIMIT_BYTES=$((PQ_GIB * 1024 * 1024 * 1024))
  [[ -x /usr/local/sbin/pq_ensure_port.sh ]] || { echo "[vless_run_temp] pq_ensure_port.sh missing" >&2; exit 1; }
  PQ_LOCK_HELD=0 MAX_IPS="$MAX_IPS" IP_TIMEOUT_SECONDS="$IP_TIMEOUT_SECONDS" \
    /usr/local/sbin/pq_ensure_port.sh "$PORT" "$LIMIT_BYTES" >/dev/null 2>&1 || { echo "[vless_run_temp] pq_ensure failed" >&2; exit 1; }
fi
...
: "${D:?请用 D=秒 vless_mktemp.sh 方式调用}"
[[ "$D" =~ ^[0-9]+$ ]] && (( D > 0 )) || { echo "❌ D 必须是正整数秒" >&2; exit 1; }
[[ -z "${PQ_GIB:-}" ]] || { [[ "$PQ_GIB" =~ ^[0-9]+$ ]] || { echo "❌ PQ_GIB 必须是非负整数" >&2; exit 1; }; }
[[ -z "${MAX_IPS:-}" ]] || { [[ "$MAX_IPS" =~ ^[0-9]+$ ]] || { echo "❌ MAX_IPS 必须是非负整数" >&2; exit 1; }; }
IP_TIMEOUT_SECONDS="${IP_TIMEOUT_SECONDS:-30}"
[[ "$IP_TIMEOUT_SECONDS" =~ ^[0-9]+$ ]] && (( IP_TIMEOUT_SECONDS > 0 )) || { echo "❌ IP_TIMEOUT_SECONDS 无效" >&2; exit 1; }
...
pick_free_port() {
  ...
  shopt -s nullglob
  for f in "${XRAY_DIR}"/vless-temp-*.meta; do
    p="$(meta_get "$f" PORT || true)"; [[ "$p" =~ ^[0-9]+$ ]] && USED_PORTS["$p"]=1
  done
  ...
  while (( port <= PORT_END )); do
    [[ -z "${USED_PORTS[$port]+x}" ]] && { echo "$port"; return 0; }
    port=$((port+1))
  done
  return 1
}
...
[[ -f "$MAIN_CFG" ]] || { echo "❌ 未找到主配置 $MAIN_CFG" >&2; exit 1; }
...
REALITY_PRIVATE_KEY="${arr[0]:-}"; REALITY_DEST="${arr[1]:-}"; REALITY_SNI="${arr[2]:-}"
[[ -n "$REALITY_PRIVATE_KEY" && -n "$REALITY_DEST" ]] || { echo "❌ 无法解析 Reality 参数" >&2; exit 1; }
[[ -n "$REALITY_SNI" ]] || REALITY_SNI="${REALITY_DEST%%:*}"

PBK="${PBK:-}"
RUNTIME_META="/root/vless_reality_runtime.meta"
[[ -z "$PBK" && -f "$RUNTIME_META" ]] && PBK="$(meta_get "$RUNTIME_META" PUBLIC_KEY || true)"
...
PORT_START="${PORT_START:-40000}"; PORT_END="${PORT_END:-50050}"
MAX_START_RETRIES="${MAX_START_RETRIES:-5}"
SERVER_ADDR="${PUBLIC_DOMAIN:-}"
[[ -z "$SERVER_ADDR" && -f "$CONF_FILE" ]] && SERVER_ADDR="$(cfg_get "$CONF_FILE" PUBLIC_DOMAIN || true)"
[[ -n "$SERVER_ADDR" && "$SERVER_ADDR" != "your.domain.com" ]] || { echo "❌ 未配置 PUBLIC_DOMAIN" >&2; exit 1; }
getent ahostsv4 "$SERVER_ADDR" >/dev/null 2>&1 || { echo "❌ PUBLIC_DOMAIN 未解析: $SERVER_ADDR" >&2; exit 1; }
domain_hits_local_ipv4 "$SERVER_ADDR" || echo "⚠️  警告：PUBLIC_DOMAIN 未命中本机 IPv4" >&2

PQ_GIB_SAFE="${PQ_GIB:-0}"; MAX_IPS_SAFE="${MAX_IPS:-0}"
POLICY_ENABLED=0; (( PQ_GIB_SAFE > 0 || MAX_IPS_SAFE > 0 )) && POLICY_ENABLED=1

UUID="$("$XRAY_BIN" uuid)"; SHORT_ID="$(openssl rand -hex 8)"
TAG="vless-temp-$(date +%Y%m%d%H%M%S)-$(openssl rand -hex 2)"
CFG="${XRAY_DIR}/${TAG}.json"; META="${XRAY_DIR}/${TAG}.meta"; UNIT="/etc/systemd/system/${TAG}.service"

ATTEMPT=1; START_OK=0; LAST_PORT=""; LAST_EXP=""
while (( ATTEMPT <= MAX_START_RETRIES )); do
  PORT="$(pick_free_port || true)"; [[ -n "$PORT" ]] || { echo "❌ 没有空闲端口" >&2; exit 1; }
  NOW="$(date +%s)"; EXP="$((NOW + D))"; LAST_PORT="$PORT"; LAST_EXP="$EXP"
  ...
  cat >"$CFG" <<CFG
{
  ...
  "inbounds": [{
    ...
    "streamSettings": {
      ...
      "realitySettings": {
        "show": false, "target": "${REALITY_DEST}", "xver": 0,
        "serverNames": ["${REALITY_SNI}"], "privateKey": "${REALITY_PRIVATE_KEY}", "shortIds": ["${SHORT_ID}"]
      }
    },
    "sniffing": { "enabled": true, "routeOnly": true, "destOverride": ["http","tls","quic"] }
  }],
...
CFG
  xray_check_config "$CFG" || { rm -f "$CFG"; echo "❌ 配置校验失败" >&2; exit 1; }
...
M
  mv -f "$META_TMP" "$META"; chmod 600 "$CFG" "$META" 2>/dev/null || true
  write_unit
  systemctl daemon-reload
  ...
  if (( POLICY_ENABLED == 1 )); then
    if ! CREATE_EPOCH="$NOW" MAX_IPS="$MAX_IPS_SAFE" IP_TIMEOUT_SECONDS="$IP_TIMEOUT_SECONDS" /usr/local/sbin/pq_add.sh "$PORT" "$PQ_GIB_SAFE" "$D" "$EXP"; then
      ...
      echo "❌ 绑定端口策略失败" >&2; exit 1
    fi
  fi

  systemctl start "$TAG".service >/dev/null 2>&1 || true
  if wait_unit_and_port_stable "$TAG".service "$PORT" 40 0.25 4; then START_OK=1; break; fi
  VLESS_LOCK_HELD=1 FORCE=1 /usr/local/sbin/vless_cleanup_one.sh "$TAG" >/dev/null 2>&1 || true
...
(( POLICY_ENABLED == 1 )) && echo "已绑定端口策略: quota=${PQ_GIB_SAFE}GiB max_ips=${MAX_IPS_SAFE} ip_timeout=${IP_TIMEOUT_SECONDS}s"
MK
...
TAG="${1:?need TAG}"; ADD_SECONDS="${2:?need seconds}"
...
NOW=$(date +%s); (( OLD_EXP > NOW )) || { echo "❌ 节点已过期" >&2; exit 1; }

NEW_D=$((OLD_D + ADD_SECONDS)); NEW_EXP=$((OLD_EXP + ADD_SECONDS))
PREV_ACTIVE=0; systemctl is-active --quiet "${TAG}.service" && PREV_ACTIVE=1 || true
mkdir -p "$MANAGED_DIR"; MANAGED_FLAG="${MANAGED_DIR}/${TAG}"

rollback_meta() { meta_set "$META" DURATION_SECONDS "$OLD_D"; meta_set "$META" EXPIRE_EPOCH "$OLD_EXP"; }
refresh_quota_lifecycle() {
  local d="$1" exp="$2"
  if [[ "$POLICY" == "1" && "$PORT" =~ ^[0-9]+$ ]]; then
    local limit_bytes=$(( PQ_GIB * 1024 * 1024 * 1024 ))
    if [[ ! -f "${PQ_DIR}/pq-${PORT}.meta" ]] && [[ -x /usr/local/sbin/pq_add.sh ]]; then
  ...
  if [[ "$PORT" =~ ^[0-9]+$ ]] && [[ -x /usr/local/sbin/pq_touch_lifecycle.sh ]] && [[ -f "${PQ_DIR}/pq-${PORT}.meta" ]]; then
...
if ! refresh_quota_lifecycle "$NEW_D" "$NEW_EXP"; then
  rollback_meta; refresh_quota_lifecycle "$OLD_D" "$OLD_EXP" || true
  echo "❌ 已回滚：配额生命周期更新失败" >&2; exit 1
fi
...
if [[ "${PAUSED:-0}" != "1" ]]; then
  ...
  if (( PREV_ACTIVE == 1 )); then
    if ! managed_restart "${TAG}.service" "$MANAGED_FLAG"; then
      ...
      echo "❌ 已回滚：续期后重启失败" >&2; exit 1
    fi
  else
    if ! systemctl start "${TAG}.service" >/dev/null 2>&1; then
      rollback_meta; refresh_quota_lifecycle "$OLD_D" "$OLD_EXP" || true
      managed_disable_now "${TAG}.service" "$MANAGED_FLAG" >/dev/null 2>&1 || true
      echo "❌ 已回滚：续期后启动失败" >&2; exit 1
    fi
  fi
  if [[ "$PORT" =~ ^[0-9]+$ ]] && ! wait_unit_and_port_stable "${TAG}.service" "$PORT" 40 0.25 4; then
    rollback_meta; refresh_quota_lifecycle "$OLD_D" "$OLD_EXP" || true
    if (( PREV_ACTIVE == 1 )); then managed_restart "${TAG}.service" "$MANAGED_FLAG" >/dev/null 2>&1 || true
    else managed_disable_now "${TAG}.service" "$MANAGED_FLAG" >/dev/null 2>&1 || true; fi
    echo "❌ 已回滚：续期后服务未稳定" >&2; exit 1
  fi
fi
echo "✅ 已续期 ${TAG}，新到期: $(TZ=Asia/Shanghai date -d "@$NEW_EXP" '+%F %T')"
EXT
...
TAG="${1:?need TAG}"; META="${XRAY_DIR}/${TAG}.meta"
exec 9>"$VLESS_LOCK"; flock -w 10 9
[[ -f "$META" ]] || { echo "❌ 未找到 $META" >&2; exit 1; }
EXP="$(meta_get "$META" EXPIRE_EPOCH || true)"; PORT="$(meta_get "$META" PORT || true)"
NOW=$(date +%s)
if [[ "$EXP" =~ ^[0-9]+$ ]] && (( EXP <= NOW )); then
  VLESS_LOCK_HELD=1 FORCE=1 /usr/local/sbin/vless_cleanup_one.sh "$TAG" >/dev/null 2>&1 || true
  echo "❌ 节点已过期，已清理" >&2; exit 1
fi
mkdir -p "$MANAGED_DIR"; MANAGED_FLAG="${MANAGED_DIR}/${TAG}"
trap 'rm -f "$MANAGED_FLAG"' EXIT
: >"$MANAGED_FLAG"
systemctl disable --now "${TAG}.service" >/dev/null 2>&1 || { rm -f "$MANAGED_FLAG"; echo "❌ 暂停失败" >&2; exit 1; }
systemctl is-active --quiet "${TAG}.service" && { echo "❌ 服务仍 active" >&2; exit 1; }
[[ "$PORT" =~ ^[0-9]+$ ]] && port_is_listening "$PORT" && { echo "❌ 端口仍在监听" >&2; exit 1; }
meta_set "$META" PAUSED 1; meta_set "$META" PAUSED_AT "$NOW"
echo "✅ 已暂停 ${TAG}（到期时间仍继续计算）"
PAUSE
...
TAG="${1:?need TAG}"
exec 9>"$VLESS_LOCK"; flock -w 10 9
META="${XRAY_DIR}/${TAG}.meta"; CFG="${XRAY_DIR}/${TAG}.json"; UNIT="/etc/systemd/system/${TAG}.service"
[[ -f "$META" ]] || { echo "❌ 未找到 $META" >&2; exit 1; }
[[ -f "$CFG" ]] || { echo "❌ 未找到 $CFG" >&2; exit 1; }
xray_check_config "$CFG" || { echo "❌ 配置校验失败" >&2; exit 1; }

OLD_PORT="$(meta_get "$META" PORT || true)"; PORT="$OLD_PORT"
CFG_PORT="$(read_port_from_cfg "$CFG" || true)"
if [[ "$CFG_PORT" =~ ^[0-9]+$ ]] && [[ "$CFG_PORT" != "$PORT" ]]; then PORT="$CFG_PORT"; meta_set "$META" PORT "$PORT"; fi

EXP="$(meta_get "$META" EXPIRE_EPOCH || true)"; NOW=$(date +%s)
POLICY="$(meta_get "$META" PQ_POLICY || true)"
PQ_GIB="$(meta_get "$META" PQ_GIB || true)"; MAX_IPS="$(meta_get "$META" PQ_MAX_IPS || true)"
IP_TIMEOUT_SECONDS="$(meta_get "$META" PQ_IP_TIMEOUT_SECONDS || true)"
DURATION_SECONDS="$(meta_get "$META" DURATION_SECONDS || true)"; CREATE_EPOCH="$(meta_get "$META" CREATE_EPOCH || true)"
[[ "$EXP" =~ ^[0-9]+$ ]] || { echo "❌ EXPIRE_EPOCH 非法" >&2; exit 1; }
(( EXP > NOW )) || { echo "❌ 节点已过期" >&2; exit 1; }

cleanup_old_policy_port "$OLD_PORT" "$PORT" "$META" "$POLICY"
[[ -f "$UNIT" ]] || { write_unit; systemctl daemon-reload >/dev/null 2>&1 || true; }

if [[ "$POLICY" == "1" && "$PORT" =~ ^[0-9]+$ ]]; then
  LIMIT_BYTES=$(( ${PQ_GIB:-0} * 1024 * 1024 * 1024 ))
  if [[ ! -f "${PQ_DIR}/pq-${PORT}.meta" ]] && [[ -x /usr/local/sbin/pq_add.sh ]]; then
    CREATE_EPOCH="$CREATE_EPOCH" MAX_IPS="${MAX_IPS:-0}" IP_TIMEOUT_SECONDS="${IP_TIMEOUT_SECONDS:-30}" \
      /usr/local/sbin/pq_add.sh "$PORT" "${PQ_GIB:-0}" "${DURATION_SECONDS:-0}" "$EXP" >/dev/null 2>&1 || { echo "❌ 端口策略重建失败" >&2; exit 1; }
  elif [[ -x /usr/local/sbin/pq_ensure_port.sh ]]; then
    PQ_LOCK_HELD=0 MAX_IPS="${MAX_IPS:-0}" IP_TIMEOUT_SECONDS="${IP_TIMEOUT_SECONDS:-30}" \
      /usr/local/sbin/pq_ensure_port.sh "$PORT" "$LIMIT_BYTES" >/dev/null 2>&1 || { echo "❌ 端口策略校验失败" >&2; exit 1; }
  fi
fi
...
if ! systemctl start "${TAG}.service" >/dev/null 2>&1; then
  mkdir -p "$MANAGED_DIR"; MANAGED_FLAG="${MANAGED_DIR}/${TAG}"
  : >"$MANAGED_FLAG"; systemctl disable --now "${TAG}.service" >/dev/null 2>&1 || true; rm -f "$MANAGED_FLAG"
  echo "❌ 恢复失败：启动失败" >&2; exit 1
fi
if [[ "$PORT" =~ ^[0-9]+$ ]] && ! wait_unit_and_port_stable "${TAG}.service" "$PORT" 40 0.25 4; then
  mkdir -p "$MANAGED_DIR"; MANAGED_FLAG="${MANAGED_DIR}/${TAG}"
  : >"$MANAGED_FLAG"; systemctl disable --now "${TAG}.service" >/dev/null 2>&1 || true; rm -f "$MANAGED_FLAG"
  echo "❌ 恢复失败：服务未稳定" >&2; exit 1
fi
meta_set "$META" PAUSED 0; meta_set "$META" PAUSED_AT 0
echo "✅ 已恢复 ${TAG}"
RESUME
chmod +x /usr/local/sbin/vless_resume.sh

########################################
# 3) Reconcile / GC / 审计 / 清空
########################################
...
exec 9>"$VLESS_LOCK"; flock -n 9 || exit 0
NOW=$(date +%s); mkdir -p "$MANAGED_DIR"

for META in "$XRAY_DIR"/vless-temp-*.meta; do
  TAG="$(meta_get "$META" TAG || true)"; PORT="$(meta_get "$META" PORT || true)"; OLD_PORT="$PORT"
  EXP="$(meta_get "$META" EXPIRE_EPOCH || true)"; PAUSED="$(meta_get "$META" PAUSED || true)"
  POLICY="$(meta_get "$META" PQ_POLICY || true)"; PQ_GIB="$(meta_get "$META" PQ_GIB || true)"
  MAX_IPS="$(meta_get "$META" PQ_MAX_IPS || true)"; IP_TIMEOUT_SECONDS="$(meta_get "$META" PQ_IP_TIMEOUT_SECONDS || true)"
  DURATION_SECONDS="$(meta_get "$META" DURATION_SECONDS || true)"; CREATE_EPOCH="$(meta_get "$META" CREATE_EPOCH || true)"
  CFG="${XRAY_DIR}/${TAG}.json"; UNIT="/etc/systemd/system/${TAG}.service"; MANAGED_FLAG="${MANAGED_DIR}/${TAG}"
  [[ -n "$TAG" && "$PORT" =~ ^[0-9]+$ && "$EXP" =~ ^[0-9]+$ ]] || continue

  if (( EXP <= NOW )); then VLESS_LOCK_HELD=1 FORCE=1 /usr/local/sbin/vless_cleanup_one.sh "$TAG" >/dev/null 2>&1 || true; continue; fi
  [[ -f "$CFG" ]] || { VLESS_LOCK_HELD=1 FORCE=1 /usr/local/sbin/vless_cleanup_one.sh "$TAG" >/dev/null 2>&1 || true; continue; }
  if ! xray_check_config "$CFG"; then
    [[ -f "$UNIT" ]] && { managed_disable_now "${TAG}.service" "$MANAGED_FLAG" >/dev/null 2>&1 || true; systemctl reset-failed "${TAG}.service" >/dev/null 2>&1 || true; }
    continue
  fi
  ...
  [[ -f "$UNIT" ]] || { write_unit "$TAG" "$CFG" "$UNIT"; systemctl daemon-reload >/dev/null 2>&1 || true; systemctl reset-failed "${TAG}.service" >/dev/null 2>&1 || true; }

  if [[ "$POLICY" == "1" && "$PORT" =~ ^[0-9]+$ ]]; then
    LIMIT_BYTES=$(( ${PQ_GIB:-0} * 1024 * 1024 * 1024 ))
    if [[ ! -f "${PQ_DIR}/pq-${PORT}.meta" && -x /usr/local/sbin/pq_add.sh ]]; then
      CREATE_EPOCH="$CREATE_EPOCH" MAX_IPS="${MAX_IPS:-0}" IP_TIMEOUT_SECONDS="${IP_TIMEOUT_SECONDS:-30}" \
        /usr/local/sbin/pq_add.sh "$PORT" "${PQ_GIB:-0}" "${DURATION_SECONDS:-0}" "$EXP" >/dev/null 2>&1 || {
        managed_disable_now "${TAG}.service" "$MANAGED_FLAG" >/dev/null 2>&1 || true; continue; }
    elif [[ -x /usr/local/sbin/pq_ensure_port.sh ]]; then
      PQ_LOCK_HELD=0 MAX_IPS="${MAX_IPS:-0}" IP_TIMEOUT_SECONDS="${IP_TIMEOUT_SECONDS:-30}" \
        /usr/local/sbin/pq_ensure_port.sh "$PORT" "$LIMIT_BYTES" >/dev/null 2>&1 || {
        managed_disable_now "${TAG}.service" "$MANAGED_FLAG" >/dev/null 2>&1 || true; continue; }
    fi
  fi
  ...
  if ! systemctl is-active --quiet "${TAG}.service"; then
    systemctl start "${TAG}.service" >/dev/null 2>&1 && \
      wait_unit_and_port_stable "${TAG}.service" "$PORT" 40 0.25 4 >/dev/null 2>&1 || true
  elif ! tcp_port_listening "$PORT"; then
    managed_restart "${TAG}.service" "$MANAGED_FLAG" && \
      wait_unit_and_port_stable "${TAG}.service" "$PORT" 40 0.25 4 >/dev/null 2>&1 || true
  fi
done
RECON
...
NOW=$(date +%s)
for META in "$XRAY_DIR"/vless-temp-*.meta; do
  TAG="$(meta_get "$META" TAG || true)"; EXPIRE_EPOCH="$(meta_get "$META" EXPIRE_EPOCH || true)"
  [[ -n "$TAG" && "$EXPIRE_EPOCH" =~ ^[0-9]+$ ]] || continue
  (( EXPIRE_EPOCH <= NOW )) && { VLESS_LOCK_HELD=1 FORCE=1 /usr/local/sbin/vless_cleanup_one.sh "$TAG" || true; }
done
GC
...
[Unit]
Description=Reconcile VLESS temp nodes after boot
Wants=nftables.service
After=network.target nftables.service
...
[Unit]
Description=Run VLESS reconcile periodically
...
systemctl disable --now vless-reconcile.timer >/dev/null 2>&1 || true
...
quota_cells() {
  local port="${1:-}" meta="${PQ_DIR}/pq-${port}.meta" qstate="none" limit="-" used="-" left="-" pct="-"
  [[ -n "$port" ]] || { printf '%s|%s|%s|%s|%s\n' "$qstate" "$limit" "$used" "$left" "$pct"; return 0; }
  if [[ -f "$meta" ]]; then
    local limit_bytes saved_used remain_bytes mode out_b in_b total_b live_used total_used left_b pct_val
    limit_bytes="$(meta_get "$meta" LIMIT_BYTES || true)"; saved_used="$(meta_get "$meta" SAVED_USED_BYTES || true)"
    remain_bytes="$(meta_get "$meta" REMAIN_BYTES || true)"; mode="$(meta_get "$meta" MODE || true)"
    [[ "$limit_bytes" =~ ^[0-9]+$ ]] || limit_bytes=0; [[ "$saved_used" =~ ^[0-9]+$ ]] || saved_used=0
    [[ "$remain_bytes" =~ ^[0-9]+$ ]] || remain_bytes="$limit_bytes"; mode="${mode:-quota}"
    ...
    if [[ "$mode" != "quota" || "$limit_bytes" == "0" ]]; then
      qstate="track"; total_used=$((saved_used+total_b)); used="$(fmt_gib "$total_used")"
    else
      ...
      (( remain_bytes == 0 )) && { total_used="$limit_bytes"; left_b=0; }
...
NAME_W=28; STATE_W=8; PORT_W=5; RDY_W=3; Q_W=5; LIMIT_W=7; USED_W=7; LEFT_W=7; USE_W=6; TTL_W=10; EXP_W=11
print_sep() { printf '%*s\n' 112 '' | tr ' ' '-'; }
render_row() {
  local name="$1" state="$2" port="$3" ready="$4" qstate="$5" limit="$6" used="$7" leftq="$8" pct="$9" ttl_left="${10}" expire_cn="${11}"
  local first=1 chunk rest="$name"
  while :; do
    if (( ${#rest} > NAME_W )); then chunk="${rest:0:NAME_W}"; rest="${rest:NAME_W}"; else chunk="$rest"; rest=""; fi
    if (( first )); then
      ...
      first=0
    else printf "%-${NAME_W}s\n" "$chunk"; fi
    [[ -z "$rest" ]] && break
  done
}

MAIN_PORT="$(read_port_from_cfg "${XRAY_DIR}/config.json" || echo 443)"
...
echo; printf '%s\n' '=== VLESS AUDIT ==='; print_sep
render_row "NAME" "STATE" "PORT" "RDY" "Q" "LIMIT" "USED" "LEFT" "USE%" "TTL" "EXPIRE"; print_sep
render_row "vless-main" "$MAIN_STATE" "$MAIN_PORT" "$MAIN_READY" "none" "-" "-" "-" "-" "-" "-"

for META in "${XRAY_DIR}"/vless-temp-*.meta; do
  TAG="$(meta_get "$META" TAG || true)"; PORT="$(meta_get "$META" PORT || true)"; EXP="$(meta_get "$META" EXPIRE_EPOCH || true)"
  PAUSED="$(meta_get "$META" PAUSED || true)"
...
(( ${#META_FILES[@]} == 0 )) && { echo "当前没有任何临时节点。"; exit 0; }
for META in "${META_FILES[@]}"; do
  TAG="$(meta_get "$META" TAG || true)"; [[ -n "$TAG" ]] || continue
...
EOF_VLESS_ENH
  ...
  # PLACEHOLDER_PQ_SYSTEM — portquota 子系统
...
mv "${OUT}.tmp" "$OUT"
if [[ -f /etc/nftables.conf ]] && ! grep -qE 'include "/etc/nftables\.d/\*\.nft"' /etc/nftables.conf 2>/dev/null; then
...
trap 'echo "❌ ${BASH_SOURCE[0]}:${LINENO}: ${BASH_COMMAND}" >&2' ERR
source /usr/local/lib/vless-common.sh
PORT="${1:-}"; BYTES="${2:-}"; MAX_IPS="${3:-${MAX_IPS:-0}}"; IP_TIMEOUT_SECONDS="${4:-${IP_TIMEOUT_SECONDS:-30}}"
[[ "$PORT" =~ ^[0-9]+$ ]] || { echo "❌ 端口非法" >&2; exit 1; }
[[ "$BYTES" =~ ^[0-9]+$ ]] || { echo "❌ bytes 非法" >&2; exit 1; }
[[ "$MAX_IPS" =~ ^[0-9]+$ ]] || MAX_IPS=0
...
TMP_RULES="$(mktemp)"; trap "rm -f '$TMP_RULES'" EXIT
{
  for chain in down_out up_in; do
    nft -a list chain inet portquota "$chain" 2>/dev/null | awk -v p="$PORT" '
      $0 ~ "comment \"pq-count-(out|in)-"p"\"" || $0 ~ "comment \"pq-drop-(out|in)-"p"\"" ||
      $0 ~ "comment \"pq-harddrop-(out|in)-"p"\"" || $0 ~ "comment \"pq-ip-refresh-(out|in)-"p"\"" ||
      $0 ~ "comment \"pq-ip-enforce-(out|in)-"p"\"" {print $NF}
    ' | while read -r h; do [[ "$h" =~ ^[0-9]+$ ]] && printf 'delete rule inet portquota %s handle %s\n' "$chain" "$h"; done
  done
...
rule_line() { nft -a list chain inet portquota "$1" 2>/dev/null | grep -F "comment \"$2\"" | head -n1 || true; }
rule_has() { local line; line="$(rule_line "$1" "$2")"; [[ -n "$line" && "$line" == *"$3"* ]]; }
quota_matches() { local line; line="$(nft list quota inet portquota "$1" 2>/dev/null | tr '\n' ' ' || true)"; [[ -n "$line" ]] && [[ "$line" == *"over ${2} bytes"* || "$line" == *"over \"${2}\" bytes"* ]]; }
set_matches() { local line; line="$(nft list set inet portquota "$1" 2>/dev/null | tr '\n' ' ' || true)"; [[ -n "$line" ]] && [[ "$line" == *"size ${2}"* ]] && [[ "$line" == *"timeout ${3}s"* ]]; }

PORT="${1:-}"; BYTES_ARG="${2:-0}"; META="${PQ_DIR}/pq-${PORT}.meta"
MAX_IPS="${3:-${MAX_IPS:-}}"; IP_TIMEOUT_SECONDS="${4:-${IP_TIMEOUT_SECONDS:-}}"
[[ "$PORT" =~ ^[0-9]+$ ]] || { echo "❌ 端口非法" >&2; exit 1; }
[[ "$BYTES_ARG" =~ ^[0-9]+$ ]] || BYTES_ARG=0
[[ -z "$MAX_IPS" && -f "$META" ]] && MAX_IPS="$(meta_get "$META" MAX_IPS || true)"
[[ -z "$IP_TIMEOUT_SECONDS" && -f "$META" ]] && IP_TIMEOUT_SECONDS="$(meta_get "$META" IP_TIMEOUT_SECONDS || true)"
[[ "$MAX_IPS" =~ ^[0-9]+$ ]] || MAX_IPS=0; [[ "$IP_TIMEOUT_SECONDS" =~ ^[0-9]+$ ]] || IP_TIMEOUT_SECONDS=30

LIMIT_BYTES="$(meta_get "$META" LIMIT_BYTES || true)"; REMAIN_BYTES="$(meta_get "$META" REMAIN_BYTES || true)"; SAVED_USED_BYTES="$(meta_get "$META" SAVED_USED_BYTES || true)"
[[ "$LIMIT_BYTES" =~ ^[0-9]+$ ]] || LIMIT_BYTES="$BYTES_ARG"; [[ "$REMAIN_BYTES" =~ ^[0-9]+$ ]] || REMAIN_BYTES="$LIMIT_BYTES"; [[ "$SAVED_USED_BYTES" =~ ^[0-9]+$ ]] || SAVED_USED_BYTES=0

OUT_B="$(get_counter_bytes "pq_out_${PORT}" || true)"; [[ "$OUT_B" =~ ^[0-9]+$ ]] || OUT_B=0
IN_B="$(get_counter_bytes "pq_in_${PORT}" || true)"; [[ "$IN_B" =~ ^[0-9]+$ ]] || IN_B=0
TOTAL_B=$((OUT_B + IN_B))
LIVE_USED="$(get_quota_used_bytes "pq_quota_${PORT}" || true)"; [[ "$LIVE_USED" =~ ^[0-9]+$ ]] || LIVE_USED="$TOTAL_B"
TOTAL_USED=$((SAVED_USED_BYTES + LIVE_USED)); (( LIMIT_BYTES > 0 && TOTAL_USED > LIMIT_BYTES )) && TOTAL_USED="$LIMIT_BYTES"
if (( LIMIT_BYTES > 0 && TOTAL_USED >= LIMIT_BYTES )); then
  [[ -f "$META" ]] && { meta_set "$META" SAVED_USED_BYTES "$LIMIT_BYTES"; meta_set "$META" REMAIN_BYTES 0; }; REMAIN_BYTES=0
fi

LOCK="$PQ_LOCK"; if [[ "${PQ_LOCK_HELD:-0}" != 1 ]]; then exec 9>"$LOCK"; flock -w 10 9; fi
NEED=0
...
if (( NEED == 1 )); then
  if (( LIVE_USED > 0 && LIMIT_BYTES > 0 )); then
    SAVED_NEW=$((SAVED_USED_BYTES + LIVE_USED)); (( SAVED_NEW > LIMIT_BYTES )) && SAVED_NEW="$LIMIT_BYTES"
    REMAIN_BYTES=$((LIMIT_BYTES - SAVED_NEW)); (( REMAIN_BYTES < 0 )) && REMAIN_BYTES=0
    [[ -f "$META" ]] && { meta_set "$META" SAVED_USED_BYTES "$SAVED_NEW"; meta_set "$META" REMAIN_BYTES "$REMAIN_BYTES"; }
  fi
...
PORT="${1:-}"; GIB="${2:-}"; SERVICE_SECONDS_ARG="${3:-${SERVICE_SECONDS:-0}}"; EXPIRE_EPOCH_ARG="${4:-${EXPIRE_EPOCH:-0}}"
CREATE_EPOCH="${CREATE_EPOCH:-$(date +%s)}"; RESET_INTERVAL_SECONDS="${RESET_INTERVAL_SECONDS:-2592000}"
MAX_IPS="${MAX_IPS:-0}"; IP_TIMEOUT_SECONDS="${IP_TIMEOUT_SECONDS:-30}"
[[ "$PORT" =~ ^[0-9]+$ ]] || { echo "❌ 端口非法" >&2; exit 1; }
[[ "$GIB" =~ ^[0-9]+$ ]] || { echo "❌ GiB 必须是非负整数" >&2; exit 1; }
for v in SERVICE_SECONDS_ARG EXPIRE_EPOCH_ARG CREATE_EPOCH RESET_INTERVAL_SECONDS MAX_IPS IP_TIMEOUT_SECONDS; do
  [[ "${!v}" =~ ^[0-9]+$ ]] || { echo "❌ ${v} 非法" >&2; exit 1; }
done

BYTES=$((GIB * 1024 * 1024 * 1024)); MODE=track; (( BYTES > 0 )) && MODE=quota
SERVICE_SECONDS="$SERVICE_SECONDS_ARG"; EXPIRE_EPOCH="$EXPIRE_EPOCH_ARG"
(( SERVICE_SECONDS > 0 && EXPIRE_EPOCH == 0 )) && EXPIRE_EPOCH=$((CREATE_EPOCH + SERVICE_SECONDS))
AUTO_RESET=0; LAST_RESET_EPOCH=0; NEXT_RESET_EPOCH=0; RESET_COUNT=0
[[ "$MODE" == quota ]] && (( SERVICE_SECONDS > RESET_INTERVAL_SECONDS && EXPIRE_EPOCH > CREATE_EPOCH )) && { AUTO_RESET=1; NEXT_RESET_EPOCH=$((CREATE_EPOCH + RESET_INTERVAL_SECONDS)); }

exec 9>"$PQ_LOCK"; flock -w 10 9
mkdir -p "$PQ_DIR"; META="${PQ_DIR}/pq-${PORT}.meta"
OLD_SAVED="$(meta_get "$META" SAVED_USED_BYTES || true)"; [[ "$OLD_SAVED" =~ ^[0-9]+$ ]] || OLD_SAVED=0
OUT_B="$(get_counter_bytes "pq_out_${PORT}" || true)"; [[ "$OUT_B" =~ ^[0-9]+$ ]] || OUT_B=0
IN_B="$(get_counter_bytes "pq_in_${PORT}" || true)"; [[ "$IN_B" =~ ^[0-9]+$ ]] || IN_B=0
TOTAL_B=$((OUT_B + IN_B))
LIVE_USED="$(get_quota_used_bytes "pq_quota_${PORT}" || true)"; [[ "$LIVE_USED" =~ ^[0-9]+$ ]] || LIVE_USED="$TOTAL_B"
USED=$((OLD_SAVED + LIVE_USED)); (( BYTES > 0 && USED > BYTES )) && USED="$BYTES"
REMAIN=$((BYTES - USED)); (( REMAIN < 0 )) && REMAIN=0
...
write_meta "${PQ_DIR}/pq-${PORT}.meta" <<M
PORT=$PORT
LIMIT_BYTES=$BYTES
LIMIT_GIB=$GIB
REMAIN_BYTES=$REMAIN
...
PQ_LOCK_HELD=1 /usr/local/sbin/pq_save.sh >/dev/null 2>&1 || true
systemctl enable --now nftables >/dev/null 2>&1 || true
if [[ "$MODE" == quota ]]; then echo "✅ 端口 $PORT: ${GIB}GiB 配额, MAX_IPS=${MAX_IPS}"
else echo "✅ 端口 $PORT: 跟踪/IP 限制, MAX_IPS=${MAX_IPS}"; fi
ADD
...
trap 'echo "❌ ${BASH_SOURCE[0]}:${LINENO}: ${BASH_COMMAND}" >&2' ERR
source /usr/local/lib/vless-common.sh
PORT="${1:-}"; GIB="${2:-}"; META="${PQ_DIR}/pq-${PORT}.meta"
[[ -f "$META" ]] || { echo "❌ 未找到 $META" >&2; exit 1; }
...
CREATE_EPOCH="$CREATE_EPOCH" SERVICE_SECONDS="$SERVICE_SECONDS" EXPIRE_EPOCH="$EXPIRE_EPOCH" \
  RESET_INTERVAL_SECONDS="$RESET_INTERVAL_SECONDS" MAX_IPS="$MAX_IPS" IP_TIMEOUT_SECONDS="$IP_TIMEOUT_SECONDS" \
  /usr/local/sbin/pq_add.sh "$PORT" "$GIB" "$SERVICE_SECONDS" "$EXPIRE_EPOCH"
SET
...
PORT="${1:?need port}"; SERVICE_SECONDS="${2:?need service seconds}"; EXPIRE_EPOCH="${3:?need expire epoch}"
META="${PQ_DIR}/pq-${PORT}.meta"; [[ -f "$META" ]] || exit 0
exec 9>"$PQ_LOCK"; flock -w 10 9

# 读取所有字段
LIMIT_BYTES="$(meta_get "$META" LIMIT_BYTES || true)"; LIMIT_GIB="$(meta_get "$META" LIMIT_GIB || true)"
REMAIN_BYTES="$(meta_get "$META" REMAIN_BYTES || true)"; SAVED_USED_BYTES="$(meta_get "$META" SAVED_USED_BYTES || true)"
MODE="$(meta_get "$META" MODE || true)"; CREATE_EPOCH_M="$(meta_get "$META" CREATE_EPOCH || true)"
RESET_INTERVAL_SECONDS="$(meta_get "$META" RESET_INTERVAL_SECONDS || true)"
AUTO_RESET="$(meta_get "$META" AUTO_RESET || true)"; LAST_RESET_EPOCH="$(meta_get "$META" LAST_RESET_EPOCH || true)"
NEXT_RESET_EPOCH="$(meta_get "$META" NEXT_RESET_EPOCH || true)"; RESET_COUNT="$(meta_get "$META" RESET_COUNT || true)"
MAX_IPS="$(meta_get "$META" MAX_IPS || true)"; IP_TIMEOUT_SECONDS_M="$(meta_get "$META" IP_TIMEOUT_SECONDS || true)"
PROTO_M="$(meta_get "$META" PROTO || true)"

# 默认值
for v in LIMIT_BYTES LIMIT_GIB SAVED_USED_BYTES; do [[ "${!v}" =~ ^[0-9]+$ ]] || eval "$v=0"; done
[[ "$REMAIN_BYTES" =~ ^[0-9]+$ ]] || REMAIN_BYTES="$LIMIT_BYTES"
[[ "$CREATE_EPOCH_M" =~ ^[0-9]+$ ]] || CREATE_EPOCH_M=0
[[ "$RESET_INTERVAL_SECONDS" =~ ^[0-9]+$ ]] || RESET_INTERVAL_SECONDS=2592000
for v in AUTO_RESET LAST_RESET_EPOCH NEXT_RESET_EPOCH RESET_COUNT; do [[ "${!v}" =~ ^[0-9]+$ ]] || eval "$v=0"; done
[[ "$MAX_IPS" =~ ^[0-9]+$ ]] || MAX_IPS=0; [[ "$IP_TIMEOUT_SECONDS_M" =~ ^[0-9]+$ ]] || IP_TIMEOUT_SECONDS_M=30
PROTO_M="${PROTO_M:-tcp}"

if [[ "$MODE" == "quota" ]] && (( LIMIT_BYTES > 0 && SERVICE_SECONDS > RESET_INTERVAL_SECONDS && EXPIRE_EPOCH > CREATE_EPOCH_M )); then
  AUTO_RESET=1
  if (( NEXT_RESET_EPOCH <= 0 )); then
    if (( LAST_RESET_EPOCH > 0 )); then NEXT_RESET_EPOCH=$((LAST_RESET_EPOCH + RESET_INTERVAL_SECONDS))
    else NEXT_RESET_EPOCH=$((CREATE_EPOCH_M + RESET_INTERVAL_SECONDS)); fi
  fi
  (( NEXT_RESET_EPOCH >= EXPIRE_EPOCH )) && NEXT_RESET_EPOCH=0
else AUTO_RESET=0; NEXT_RESET_EPOCH=0; fi
...
RESET_COUNT=$RESET_COUNT
MAX_IPS=$MAX_IPS
IP_TIMEOUT_SECONDS=$IP_TIMEOUT_SECONDS_M
PROTO=$PROTO_M
M
TOUCH
...
exec 9>"$PQ_LOCK"; flock -n 9 || exit 0
NOW="$(date +%s)"; CHANGED=0

for META in "${PQ_DIR}"/pq-*.meta; do
  PORT="$(meta_get "$META" PORT || true)"; LIMIT_BYTES="$(meta_get "$META" LIMIT_BYTES || true)"
  LIMIT_GIB="$(meta_get "$META" LIMIT_GIB || true)"; MODE="$(meta_get "$META" MODE || true)"
  CREATE_EPOCH="$(meta_get "$META" CREATE_EPOCH || true)"; SERVICE_SECONDS="$(meta_get "$META" SERVICE_SECONDS || true)"
  EXPIRE_EPOCH="$(meta_get "$META" EXPIRE_EPOCH || true)"; RESET_INTERVAL_SECONDS="$(meta_get "$META" RESET_INTERVAL_SECONDS || true)"
  AUTO_RESET="$(meta_get "$META" AUTO_RESET || true)"; LAST_RESET_EPOCH="$(meta_get "$META" LAST_RESET_EPOCH || true)"
  NEXT_RESET_EPOCH="$(meta_get "$META" NEXT_RESET_EPOCH || true)"; RESET_COUNT="$(meta_get "$META" RESET_COUNT || true)"
  MAX_IPS="$(meta_get "$META" MAX_IPS || true)"; IP_TIMEOUT_SECONDS="$(meta_get "$META" IP_TIMEOUT_SECONDS || true)"

  [[ "$PORT" =~ ^[0-9]+$ && "$LIMIT_BYTES" =~ ^[0-9]+$ ]] || continue
  [[ "$MODE" == quota ]] || continue
  for v in AUTO_RESET SERVICE_SECONDS EXPIRE_EPOCH RESET_INTERVAL_SECONDS NEXT_RESET_EPOCH RESET_COUNT MAX_IPS IP_TIMEOUT_SECONDS; do
    [[ "${!v}" =~ ^[0-9]+$ ]] || eval "$v=0"
  done
  [[ "$RESET_INTERVAL_SECONDS" -gt 0 ]] || RESET_INTERVAL_SECONDS=2592000
  [[ "$IP_TIMEOUT_SECONDS" -gt 0 ]] || IP_TIMEOUT_SECONDS=30
  ...
  while (( NEXT_RESET_EPOCH > 0 && NEXT_RESET_EPOCH <= NOW && NEXT_RESET_EPOCH < EXPIRE_EPOCH )); do
    PQ_LOCK_HELD=1 MAX_IPS="$MAX_IPS" IP_TIMEOUT_SECONDS="$IP_TIMEOUT_SECONDS" \
      /usr/local/sbin/pq_apply_port.sh "$PORT" "$LIMIT_BYTES" "$MAX_IPS" "$IP_TIMEOUT_SECONDS" || break
    LAST_RESET_EPOCH="$NEXT_RESET_EPOCH"
    NEXT_RESET_EPOCH=$((NEXT_RESET_EPOCH + RESET_INTERVAL_SECONDS))
    RESET_COUNT=$((RESET_COUNT + 1)); DID_RESET=1
  done
  (( NEXT_RESET_EPOCH >= EXPIRE_EPOCH )) && NEXT_RESET_EPOCH=0
...
source /usr/local/lib/vless-common.sh
exec 9>"$PQ_LOCK"; flock -n 9 || exit 0
[[ -x /usr/local/sbin/pq_ensure_port.sh ]] || { echo "[pq_reconcile] missing pq_ensure_port.sh" >&2; exit 1; }
for META in "${PQ_DIR}"/pq-*.meta; do
  PORT="$(meta_get "$META" PORT || true)"; MAX_IPS="$(meta_get "$META" MAX_IPS || true)"; IP_TIMEOUT_SECONDS="$(meta_get "$META" IP_TIMEOUT_SECONDS || true)"
  [[ "$PORT" =~ ^[0-9]+$ ]] || continue
  [[ "$MAX_IPS" =~ ^[0-9]+$ ]] || MAX_IPS=0; [[ "$IP_TIMEOUT_SECONDS" =~ ^[0-9]+$ ]] || IP_TIMEOUT_SECONDS=30
  PQ_LOCK_HELD=1 MAX_IPS="$MAX_IPS" IP_TIMEOUT_SECONDS="$IP_TIMEOUT_SECONDS" /usr/local/sbin/pq_ensure_port.sh "$PORT" 0 >/dev/null 2>&1 || true
done
RECON
...
trap 'echo "❌ ${BASH_SOURCE[0]}:${LINENO}: ${BASH_COMMAND}" >&2' ERR
source /usr/local/lib/vless-common.sh
PORT="${1:-}"; [[ "$PORT" =~ ^[0-9]+$ ]] || { echo "❌ 用法: pq_del.sh <端口>" >&2; exit 1; }
exec 9>"$PQ_LOCK"; flock -w 10 9
if ! nft list table inet portquota >/dev/null 2>&1; then rm -f "${PQ_DIR}/pq-${PORT}.meta"; exit 0; fi
TMP_RULES="$(mktemp)"; trap "rm -f '$TMP_RULES'" EXIT
{
  for chain in down_out up_in; do
    nft -a list chain inet portquota "$chain" 2>/dev/null | awk -v p="$PORT" '
...
[[ -s "$TMP_RULES" ]] && nft -f "$TMP_RULES"
rm -f "${PQ_DIR}/pq-${PORT}.meta"
PQ_LOCK_HELD=1 /usr/local/sbin/pq_save.sh >/dev/null 2>&1 || true
DEL
...
get_ipset_count() { nft list set inet portquota "$1" 2>/dev/null | awk 'BEGIN{c=0} /elements = \{/ {line=$0; sub(/^.*elements = \{ */, "", line); sub(/ *\}.*$/, "", line); n=split(line,a,/,/); for(i=1;i<=n;i++) if(a[i] ~ /([0-9]{1,3}\.){3}[0-9]{1,3}/) c++;} END{print c+0}'; }

printf "%-6s %-7s %-9s %-9s %-7s %-5s %-5s %-5s %-5s %-5s %-11s %-11s\n" PORT STATE TOTALGiB LIMITGiB USE% IPCUR IPMAX IPTO RESET COUNT NEXT_RESET EXPIRE
for META in "${PQ_DIR}"/pq-*.meta; do
  PORT="$(meta_get "$META" PORT || true)"; LIMIT_BYTES="$(meta_get "$META" LIMIT_BYTES || true)"
  SAVED_USED="$(meta_get "$META" SAVED_USED_BYTES || true)"; REMAIN_BYTES="$(meta_get "$META" REMAIN_BYTES || true)"
  MODE="$(meta_get "$META" MODE || true)"; AUTO_RESET="$(meta_get "$META" AUTO_RESET || true)"
  NEXT_RESET_EPOCH="$(meta_get "$META" NEXT_RESET_EPOCH || true)"; RESET_COUNT="$(meta_get "$META" RESET_COUNT || true)"
  EXPIRE_EPOCH="$(meta_get "$META" EXPIRE_EPOCH || true)"; MAX_IPS="$(meta_get "$META" MAX_IPS || true)"
  ...
  if [[ "$MODE" == quota && "$LIMIT_BYTES" -gt 0 ]]; then
    LIVE_USED="$(get_quota_used_bytes "pq_quota_${PORT}" || true)"; [[ "$LIVE_USED" =~ ^[0-9]+$ ]] || LIVE_USED="$TOTAL_B"
    TOTAL_USED=$((SAVED_USED + LIVE_USED)); (( TOTAL_USED > LIMIT_BYTES )) && TOTAL_USED="$LIMIT_BYTES"
    (( REMAIN_BYTES == 0 )) && TOTAL_USED="$LIMIT_BYTES"
    LIMIT_GIB="$(awk -v b="$LIMIT_BYTES" 'BEGIN{printf "%.2f",b/1024/1024/1024}')"; TOTAL_GIB="$(awk -v b="$TOTAL_USED" 'BEGIN{printf "%.2f",b/1024/1024/1024}')"
    ...
    TOTAL_USED=$((SAVED_USED + TOTAL_B)); TOTAL_GIB="$(awk -v b="$TOTAL_USED" 'BEGIN{printf "%.2f",b/1024/1024/1024}')"; LIMIT_GIB="-"; PCT="-"; STATE=track
  fi
...
for META in "${PQ_DIR}"/pq-*.meta; do
  PORT="$(meta_get "$META" PORT || true)"; LIMIT_BYTES="$(meta_get "$META" LIMIT_BYTES || true)"
  SAVED_USED_BYTES="$(meta_get "$META" SAVED_USED_BYTES || true)"; MODE="$(meta_get "$META" MODE || true)"
  [[ "$PORT" =~ ^[0-9]+$ ]] || continue
  [[ "$LIMIT_BYTES" =~ ^[0-9]+$ ]] || LIMIT_BYTES=0; [[ "$SAVED_USED_BYTES" =~ ^[0-9]+$ ]] || SAVED_USED_BYTES=0
  OUT_B="$(get_counter_bytes "pq_out_${PORT}" || true)"; [[ "$OUT_B" =~ ^[0-9]+$ ]] || OUT_B=0
  IN_B="$(get_counter_bytes "pq_in_${PORT}" || true)"; [[ "$IN_B" =~ ^[0-9]+$ ]] || IN_B=0
  TOTAL_B=$((OUT_B + IN_B))
  if [[ "$MODE" == quota && "$LIMIT_BYTES" -gt 0 ]]; then
    LIVE_USED="$(get_quota_used_bytes "pq_quota_${PORT}" || true)"; [[ "$LIVE_USED" =~ ^[0-9]+$ ]] || LIVE_USED="$TOTAL_B"
  else LIVE_USED="$TOTAL_B"; fi
  NEW_SAVED=$((SAVED_USED_BYTES + LIVE_USED))
  # 修复: track 模式下不用 LIMIT_BYTES 截断
  if (( LIMIT_BYTES > 0 && NEW_SAVED > LIMIT_BYTES )); then NEW_SAVED="$LIMIT_BYTES"; fi
...
[Unit]
Description=Reconcile nftables portquota rules
...
[Unit]
Description=Reconcile nftables portquota periodically
...
systemctl disable --now pq-reset.timer >/dev/null 2>&1 || true
systemctl disable --now pq-reconcile.timer >/dev/null 2>&1 || true
...
可用命令一览：
  ...
  3) bash /root/vless_temp_audit_ipv4_all.sh
     ...
     PQ_GIB=50 D=600 vless_mktemp.sh
     MAX_IPS=3 PQ_GIB=50 D=600 vless_mktemp.sh
  4) vless_extend.sh / vless_pause.sh / vless_resume.sh / vless_audit.sh
  5) pq_add.sh / pq_set.sh / pq_audit.sh / pq_del.sh
==================================================
DONE
}
