...
1) 新建一个临时 VLESS 节点（例如 600 秒）：
   ...
   IP_LIMIT=3 IP_STICKY_SECONDS=120 D=600 vless_mktemp.sh

   # 配额和 IP 限制可同时开启
   PQ_GIB=50 IP_LIMIT=3 IP_STICKY_SECONDS=120 D=600 vless_mktemp.sh
...
pq_counter_bytes() {
  local obj="$1"
  nft -j list counter inet portquota "$obj" 2>/dev/null | python3 - <<'PYJSON'
import json, sys
try:
    data = json.load(sys.stdin)
    for item in data.get('nftables', []):
        if isinstance(item, dict) and 'counter' in item:
            val = item['counter'].get('bytes')
...
pq_quota_used_bytes() {
  local obj="$1"
  nft -j list quota inet portquota "$obj" 2>/dev/null | python3 - <<'PYJSON'
import json, sys
try:
    data = json.load(sys.stdin)
    for item in data.get('nftables', []):
        if isinstance(item, dict) and 'quota' in item:
            val = item['quota'].get('used')
            if isinstance(val, int):
                print(val)
                raise SystemExit(0)
except Exception:
...
pq_get_live_used_bytes() {
  ...
  [[ "$quota_b" =~ ^[0-9]+$ ]] || quota_b=0

  live_b="$total_b"
  if (( quota_b > live_b )); then
    live_b="$quota_b"
  fi
...
for META in /etc/portquota/pq-*.meta; do
  ...
  if (( LIVE_USED_BYTES > 0 )); then
    ...
    NEW_LIMIT_BYTES=$((ORIGINAL_LIMIT_BYTES - NEW_SAVED_USED_BYTES))
    ...
    if PQ_LOCK_HELD=1 /usr/local/sbin/pq_apply_port.sh "$PORT" "$NEW_LIMIT_BYTES"; then
      CHANGED=1
      ...
      echo "$(date '+%F %T %Z') [pq-state] port=$PORT reapply failed" >> "$LOG"
    fi
  fi
done
...
[Unit]
Description=Persist portquota live usage and save nftables snapshot
...
[Unit]
Description=Periodically persist portquota state
...
5) 手动持久化当前开机期间的 live used（正常重启/关机前会自动执行）：
   pq_save_state.sh   # 立即把当前 live used 并入累计并重建剩余额度
...
4) TCP 配额（nftables + 5 分钟持久化累计用量，双向合计）：
   # 主节点配额
   pq_add.sh 443 500
