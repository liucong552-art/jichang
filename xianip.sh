...
[Service]
Type=oneshot
RuntimeDirectory=vless-reality
...
rows.sort(key=lambda r: int(r[0]) if r[0].isdigit() else 10**9)
...
', ' ')
    ...
    for paragraph in text.split('\n'):
        ...
        curw = 0
        for ch in paragraph:
            chw = 2 if (not unicodedata.combining(ch) and unicodedata.east_asian_width(ch) in ('W', 'F')) else 1
            if cur and curw + chw > width:
                lines.append(cur)
                cur = ch
                curw = chw
            else:
...
def pad(text: str, width: int, align='left'):
    text = text or ''
    diff = max(0, width - cell_width(text))
    if align == 'right':
        return ' ' * diff + text
    if align == 'center':
        left = diff // 2
...
term = shutil.get_terminal_size((170, 40)).columns
owner_w = 24 + min(20, max(0, term - 140))
cols = [
    ('PORT', 7, 'right', False),
    ('OWNER', owner_w, 'left', True),
    ('STATE', 10, 'center', False),
    ('LIMIT', 12, 'right', False),
    ('USED', 12, 'right', False),
    ('LEFT', 12, 'right', False),
    ('USE%', 8, 'right', False),
    ('RESET', 7, 'center', False),
    ('NEXT_RESET(BJ)', 19, 'center', False),
]

title = f'端口配额审计  共 {len(rows)} 项'
print(title)
top = '┌' + '┬'.join('─' * (w + 2) for _, w, _, _ in cols) + '┐'
mid = '├' + '┼'.join('─' * (w + 2) for _, w, _, _ in cols) + '┤'
head = '╞' + '╪'.join('═' * (w + 2) for _, w, _, _ in cols) + '╡'
bot = '└' + '┴'.join('─' * (w + 2) for _, w, _, _ in cols) + '┘'
print(top)
print('│ ' + ' │ '.join(pad(name, width, 'center') for name, width, _, _ in cols) + ' │')
print(head)
for idx, row in enumerate(rows):
    wrapped_cols = []
    max_lines = 1
    for value, (_, width, _, wrap) in zip(row, cols):
        lines = wrap_text(value, width) if wrap else [value]
        wrapped_cols.append(lines)
        max_lines = max(max_lines, len(lines))
    for line_idx in range(max_lines):
        pieces = []
        for lines, (_, width, align, _) in zip(wrapped_cols, cols):
            value = lines[line_idx] if line_idx < len(lines) else ''
            pieces.append(pad(value, width, align))
        print('│ ' + ' │ '.join(pieces) + ' │')
    if idx != len(rows) - 1:
        print(mid)
print(bot)
...
term = shutil.get_terminal_size((190, 40)).columns
name_w = 28 + min(24, max(0, term - 165))
cols = [
    ('NAME', name_w, 'left', True),
    ('STATE', 10, 'center', False),
    ('PORT', 6, 'right', False),
    ('LISTEN', 6, 'center', False),
    ('QUOTA', 10, 'center', False),
    ('LIMIT', 11, 'right', False),
    ('USED', 11, 'right', False),
    ('LEFT', 11, 'right', False),
    ('USE%', 7, 'right', False),
    ('TTL', 16, 'center', False),
    ('EXPIRE(BJ)', 19, 'center', False),
    ('IPLIM', 6, 'right', False),
    ('IP_ACTIVE', 9, 'right', False),
    ('STICKY', 6, 'right', False),
]

title = f'VLESS 节点审计  共 {len(rows)} 项'
print(title)
top = '┌' + '┬'.join('─' * (w + 2) for _, w, _, _ in cols) + '┐'
mid = '├' + '┼'.join('─' * (w + 2) for _, w, _, _ in cols) + '┤'
head = '╞' + '╪'.join('═' * (w + 2) for _, w, _, _ in cols) + '╡'
