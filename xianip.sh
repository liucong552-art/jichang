...
rule_line() {
  ...
  nft -a list chain inet portquota "$chain" 2>/dev/null | grep -F "comment \"$comment\"" | head -n1 || true
}
rule_matches() {
  local chain="$1" comment="$2" expect1="${3:-}" expect2="${4:-}" line
  line="$(rule_line "$chain" "$comment")"
  [[ -n "$line" ]] || return 1
...
BYTES="${2:-}"
MAX_IPS="${3:-${MAX_IPS:-0}}"
IP_TIMEOUT_SECONDS="${4:-${IP_TIMEOUT_SECONDS:-90}}"
[[ "$PORT" =~ ^[0-9]+$ ]] || { echo "❌ 端口非法" >&2; exit 1; }
[[ "$BYTES" =~ ^[0-9]+$ ]] || { echo "❌ bytes 必须是非负整数" >&2; exit 1; }
[[ "$MAX_IPS" =~ ^[0-9]+$ ]] || MAX_IPS=0
[[ "$IP_TIMEOUT_SECONDS" =~ ^[0-9]+$ ]] || IP_TIMEOUT_SECONDS=90
...
if [[ "${PQ_LOCK_HELD:-0}" != "1" ]]; then exec 9>"$LOCK"; flock -w 10 9; fi
NEED_REBUILD=0
nft list table inet portquota >/dev/null 2>&1 || NEED_REBUILD=1
nft list chain inet portquota down_out >/dev/null 2>&1 || NEED_REBUILD=1
nft list chain inet portquota up_in >/dev/null 2>&1 || NEED_REBUILD=1
obj_exists counter "pq_out_$PORT" || NEED_REBUILD=1
obj_exists counter "pq_in_$PORT" || NEED_REBUILD=1
rule_matches down_out "pq-count-out-$PORT" "sport $PORT" "counter name \"pq_out_$PORT\"" || NEED_REBUILD=1
rule_matches up_in "pq-count-in-$PORT" "dport $PORT" "counter name \"pq_in_$PORT\"" || NEED_REBUILD=1
if (( BYTES > 0 )); then
  obj_exists quota "pq_quota_$PORT" || NEED_REBUILD=1
  quota_matches "pq_quota_$PORT" "$BYTES" || NEED_REBUILD=1
  rule_matches down_out "pq-drop-out-$PORT" "sport $PORT" "quota name \"pq_quota_$PORT\" drop" || NEED_REBUILD=1
  rule_matches up_in "pq-drop-in-$PORT" "dport $PORT" "quota name \"pq_quota_$PORT\" drop" || NEED_REBUILD=1
else
  obj_exists quota "pq_quota_$PORT" && NEED_REBUILD=1
  [[ -z "$(rule_line down_out "pq-drop-out-$PORT")" ]] || NEED_REBUILD=1
  [[ -z "$(rule_line up_in "pq-drop-in-$PORT")" ]] || NEED_REBUILD=1
fi
if (( MAX_IPS > 0 )); then
  obj_exists set "pq_ip_$PORT" || NEED_REBUILD=1
  set_matches "pq_ip_$PORT" "$MAX_IPS" "$IP_TIMEOUT_SECONDS" || NEED_REBUILD=1
  rule_matches up_in "pq-ip-refresh-in-$PORT" "dport $PORT" "update @pq_ip_$PORT { ip saddr timeout ${IP_TIMEOUT_SECONDS}s }" || NEED_REBUILD=1
  rule_matches up_in "pq-ip-enforce-in-$PORT" "dport $PORT" "ip saddr != @pq_ip_$PORT drop" || NEED_REBUILD=1
  rule_matches down_out "pq-ip-refresh-out-$PORT" "sport $PORT" "update @pq_ip_$PORT { ip daddr timeout ${IP_TIMEOUT_SECONDS}s }" || NEED_REBUILD=1
  rule_matches down_out "pq-ip-enforce-out-$PORT" "sport $PORT" "ip daddr != @pq_ip_$PORT drop" || NEED_REBUILD=1
else
  obj_exists set "pq_ip_$PORT" && NEED_REBUILD=1
  [[ -z "$(rule_line up_in "pq-ip-refresh-in-$PORT")" ]] || NEED_REBUILD=1
  [[ -z "$(rule_line up_in "pq-ip-enforce-in-$PORT")" ]] || NEED_REBUILD=1
  [[ -z "$(rule_line down_out "pq-ip-refresh-out-$PORT")" ]] || NEED_REBUILD=1
  [[ -z "$(rule_line down_out "pq-ip-enforce-out-$PORT")" ]] || NEED_REBUILD=1
fi
