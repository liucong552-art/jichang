...
SCHEMAS = {
    "vless": [
        {"name": "NAME",  "min":  8, "ideal": 26, "max": 64, "align": "left",  "weight": 20},
        {"name": "STATE", "min":  6, "ideal":  8, "max": 12, "align": "left",  "weight":  2},
        {"name": "PORT",  "min":  5, "ideal":  5, "max":  5, "align": "right", "weight":  1},
        {"name": "LISN",  "min":  4, "ideal":  4, "max":  6, "align": "left",  "weight":  1},
        {"name": "QUOTA", "min":  5, "ideal":  6, "max": 10, "align": "left",  "weight":  1},
        {"name": "LIMIT", "min":  7, "ideal": 10, "max": 14, "align": "right", "weight":  1},
        {"name": "USED",  "min":  7, "ideal": 10, "max": 14, "align": "right", "weight":  1},
        {"name": "LEFT",  "min":  7, "ideal": 10, "max": 14, "align": "right", "weight":  1},
        {"name": "USE%",  "min":  4, "ideal":  5, "max":  7, "align": "right", "weight":  1},
        {"name": "TTL",   "min":  6, "ideal": 12, "max": 18, "align": "left",  "weight":  8},
        {"name": "EXPBJ", "min":  8, "ideal": 19, "max": 22, "align": "left",  "weight": 10},
        {"name": "IPLM",  "min":  4, "ideal":  4, "max":  6, "align": "right", "weight":  1},
        {"name": "IPACT", "min":  5, "ideal":  5, "max":  7, "align": "right", "weight":  1},
        {"name": "STKY",  "min":  4, "ideal":  5, "max":  7, "align": "right", "weight":  1},
    ],
    "pq": [
        {"name": "PORT",   "min":  5, "ideal":  5, "max":  5, "align": "right", "weight":  1},
        {"name": "OWNER",  "min": 12, "ideal": 34, "max": 76, "align": "left",  "weight": 20},
        {"name": "STATE",  "min":  6, "ideal":  8, "max": 12, "align": "left",  "weight":  2},
        {"name": "LIMIT",  "min":  7, "ideal": 10, "max": 14, "align": "right", "weight":  1},
        {"name": "USED",   "min":  7, "ideal": 10, "max": 14, "align": "right", "weight":  1},
        {"name": "LEFT",   "min":  7, "ideal": 10, "max": 14, "align": "right", "weight":  1},
        ...
        {"name": "NEXTBJ", "min":  8, "ideal": 19, "max": 22, "align": "left",  "weight": 10},
    ],
}
...
def take_prefix(text: str, width: int):
    ...
    while idx < len(text):
        ...
        if ch == "\n":
            ...
            break
        w = char_width(ch)
        if out and used + w > width:
            break
        if not out and w > width:
            break
        out.append(ch)
        used += w
        idx += 1
...
def wrap_cell(text: str, width: int):
    ...
    rest = text
    while rest:
        chunk, rest = take_prefix(rest, width)
        if not chunk and rest:
            chunk, rest = rest[0], rest[1:]
        if not chunk:
            break
        parts.append(chunk)
    return parts or ["-"]
...
def pad(text: str, width: int, align: str):
    ...
    if text_width(text) > width:
        text = take_prefix(text, width)[0]
    pad_len = max(0, width - text_width(text))
    spaces = " " * pad_len
    return spaces + text if align == "right" else text + spaces
...
def allocate_widths(schema):
    ...
    widths = ideals[:]
    available = max(sum(mins), terminal_columns() - (3 * len(schema) + 1))
