...
need_basic_tools() {
  ...
  apt-get install -y --no-install-recommends \
    ca-certificates curl wget openssl python3 socat cron nftables iproute2 util-linux coreutils grep sed gawk logrotate systemd
}
...
HY2_RENEW_HOOK="/usr/local/lib/hy2/renew-reload.sh"
...
ensure_acmesh_zerossl() {
  ...
  [[ -x "$acme_bin" ]] || hy2_die "acme.sh 安装失败"

  "$acme_bin" --set-default-ca --server zerossl >/dev/null 2>&1 || true
  "$acme_bin" --register-account -m "$ACME_EMAIL" --server zerossl >/dev/null 2>&1 || true
}
...
issue_or_renew_cert_zerossl() {
  ...
  "$acme_bin" --set-default-ca --server zerossl >/dev/null 2>&1 || true
  "$acme_bin" --register-account -m "$ACME_EMAIL" --server zerossl >/dev/null 2>&1 || true
  "$acme_bin" --issue --standalone --server zerossl -d "$domain"
  "$acme_bin" --install-cert -d "$domain" \
    --key-file "$key" \
    --fullchain-file "$crt" \
    --reloadcmd "bash ${HY2_RENEW_HOOK}"
