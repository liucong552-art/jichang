...
check_ubuntu2204() {
  ...
  [[ "$os_id" == "ubuntu" && "$codename" == "jammy" ]] || {
    echo "❌ 本脚本仅适用于 Ubuntu 22.04 LTS (jammy)，当前: ${os_id:-未知}/${codename:-未知}" >&2
    exit 1
  }
}
...
need_basic_tools() {
  ...
  apt-get install -y --no-install-recommends \
    ca-certificates curl wget openssl python3 cron socat nftables iproute2 util-linux coreutils grep sed gawk logrotate systemd kmod
}
...
hy2_require_root_ubuntu2204() {
  ...
  local os_id
  os_id="$(grep -E '^ID=' /etc/os-release 2>/dev/null | cut -d= -f2 | tr -d '"')"
  [[ "$os_id" == "ubuntu" && "$codename" == "jammy" ]] || hy2_die "仅支持 Ubuntu 22.04 LTS (jammy)，当前: ${os_id:-未知}/${codename:-未知}"
}
...
os_id="$(grep -E '^ID=' /etc/os-release 2>/dev/null | cut -d= -f2 | tr -d '"')"
[[ "$os_id" == "ubuntu" && "$codename" == "jammy" ]] || {
  echo "❌ 仅支持 Ubuntu 22.04 LTS (jammy)，当前: ${os_id:-未知}/${codename:-未知}" >&2
  exit 1
}
...
apt-get update -o Acquire::Retries=3
# Ubuntu 22.04 使用官方 HWE 内核元包，不写入 Debian backports 源。
apt-get install -y linux-generic-hwe-22.04 || apt-get install -y linux-generic
...
main() {
  ...
  cat <<'DONE'
==================================================
✅ 新版 HY2 受管系统已写入完成（Ubuntu 22.04 LTS）
