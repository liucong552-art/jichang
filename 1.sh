...
need_basic_tools() {
  ...
  apt-get install -y --no-install-recommends \
    ca-certificates curl wget openssl python3 certbot nftables iproute2 util-linux coreutils grep sed gawk logrotate systemd
}
...
hy2_port_is_listening_udp() {
  local port="$1"
  ss -lunH 2>/dev/null | awk -v p="$port" '$5 ~ ":"p"$" {found=1} END{exit !found}'
}
