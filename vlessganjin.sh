...
install_xray_from_official() {
  local tmp
  tmp="$(mktemp /tmp/xray-install-release.XXXXXX.sh)"
  echo "⬇ 下载 Xray 官方安装脚本..."
  curl4 -L "https://github.com/XTLS/Xray-install/raw/main/install-release.sh" -o "$tmp"
  chmod +x "$tmp"
  echo "⚙ 安装 / 更新 Xray-core..."
  bash "$tmp" install --without-geodata
  rm -f "$tmp"
