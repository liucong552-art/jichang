...
#   1. DOMAIN：必改，改成新域名
#   2. Cloudflare DNS：A 记录指向新 VPS IP，并开启小黄云
#   3. VPS 防火墙 / 云安全组：放行 TLS_PORT，例如 443 / 2053 / 8443
#
# 可选修改：
#   TLS_PORT   ：公网 TLS/WSS 端口，小黄云只能用 443/2053/2083/2087/2096/8443
#   XRAY_PORT  ：Xray 本地端口，只监听 127.0.0.1，别和本机端口冲突即可
#   HTTP_PORT  ：HTTP 占位端口，默认 80；如果 80 被 Caddy/Nginx/宝塔占用，可设为 0
#   UUID       ：不填则随机生成；想固定可手动传入
#   WSPATH     ：不填则随机生成；想固定可手动传入，例如 WSPATH="/abc123"
#   CF_STRICT  ：默认 1，强制检查 Cloudflare 小黄云端口；灰云直连可设为 0
#
# 默认交互运行：
#   bash <(curl -fsSL https://raw.githubusercontent.com/liucong552-art/jichang/refs/heads/main/cfyouxuanduankou.sh)
#
# 不交互运行：
#   DOMAIN="hy2.liucna.com" TLS_PORT=2053 XRAY_PORT=20000 HTTP_PORT=0 bash <(curl -fsSL https://raw.githubusercontent.com/liucong552-art/jichang/refs/heads/main/cfyouxuanduankou.sh)
# ==========================================================
...
NGINX_CONFIG="/etc/nginx/sites-available/xray-wss.conf"
NGINX_LINK="/etc/nginx/sites-enabled/xray-wss.conf"
LINK_FILE="/root/cf-wss-vless-link.txt"
...
check_port_used_by_other_service() {
    ...
    if ! command -v ss >/dev/null 2>&1; then
        echo "⚠️ 当前系统没有 ss 命令，跳过 ${name}=${port} 的预检查。"
        return 0
    fi
    ...
    echo
    echo "请换一个端口后重新运行。"
    echo "例如 80 被占用时，可使用 HTTP_PORT=0。"
    exit 1
}
...
echo "更新软件源并安装依赖..."
apt update
DEBIAN_FRONTEND=noninteractive apt install -y curl wget unzip openssl nginx ca-certificates python3 iproute2 iptables-persistent netfilter-persistent

echo
echo "修复 Oracle/Ubuntu/Debian 默认 iptables：放行需要的 TCP 端口，避免被 REJECT 拦截..."
...
echo "5. Cloudflare SSL/TLS 模式建议使用 Full。"
echo "6. 如果继续使用本脚本的 ssl_verify_client on，Cloudflare 后台需要开启 Authenticated Origin Pulls。"
echo "7. 每次运行默认会生成新的 UUID 和 WSPATH，客户端要导入本次输出的新链接。"
echo
