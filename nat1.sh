...
# 用法：
#   bash nat.sh init
#   bash nat.sh add <name> <VPS_IP或域名> <VPS_WG_ADDR> '<VPS_WG_PUBLIC_KEY>'
...
示例：
  ...
  bash $0 add vps-2 hk.example.com 10.66.66.3/32 'PUBLIC_KEY'
  bash $0 del vps-1
  bash $0 list
  bash $0 status
EOF_USAGE
...
EOF_SYSCTL
  ...
  echo "bash $0 add vps-2 hk.example.com 10.66.66.3/32 'VPS_WG_PUBLIC_KEY'"
}
...
main(){
  ...
  case "$cmd" in
    init)
      ...
      ;;
    add)
      [[ $# -eq 5 ]] || fail "用法: bash $0 add <name> <VPS_IP或域名> <VPS_WG_ADDR> '<VPS_WG_PUBLIC_KEY>'"
      cmd_add "$2" "$3" "$4" "$5"
      ;;
    del)
